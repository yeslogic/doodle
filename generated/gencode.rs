...
#[derive(Debug, Clone)]
pub enum opentype_gpos_table_lookup_list_link_lookups_link_subtables_link { ChainedSequenceContext(opentype_common_chained_sequence_context), CursivePos(opentype_layout_cursive_pos), MarkBasePos(opentype_layout_mark_base_pos), MarkLigPos(opentype_layout_mark_lig_pos), MarkMarkPos, PairPos(opentype_layout_pair_pos), PosExtension, SequenceContext(opentype_common_sequence_context), SinglePos(opentype_layout_single_pos) }
...
#[derive(Debug, Clone)]
pub enum opentype_gsub_table_lookup_list_link_lookups_link_subtables_link { AlternateSubst(opentype_layout_alternate_subst), ChainedSequenceContext(opentype_common_chained_sequence_context), LigatureSubst(opentype_layout_ligature_subst), MultipleSubst(opentype_layout_multiple_subst), ReverseChainSingleSubst(opentype_layout_reverse_chain_single_subst), SequenceContext(opentype_common_sequence_context), SingleSubst(opentype_layout_single_subst), SubstExtension }
...
let r#where = ((|| PResult::Ok((Decoder79(_input))?))())?;
...
let next_elem = (Decoder308(_input))?;
accum.push(next_elem);
}
}
PResult::Ok(accum)
...
let gzip_raw = (Decoder285(_input))?;
let mut accum = Vec::new();
...
let eoi = ((|| PResult::Ok((Decoder207(_input))?))())?;
...
let signature = ((|| PResult::Ok((Decoder136(_input))?))())?;
...
let length = ((|| PResult::Ok((Decoder107(_input))?))())?;
let data = ((|| PResult::Ok({
...
tiff_main_byte_order::le(..) => {
let num_fields = ((|| PResult::Ok((Decoder119(_input))?))())?;
let fields = ((|| PResult::Ok({
let mut accum = Vec::new();
for _ in 0..num_fields {
accum.push({
let tag = ((|| PResult::Ok((Decoder119(_input))?))())?;
let r#type = ((|| PResult::Ok((Decoder119(_input))?))())?;
let length = ((|| PResult::Ok((Decoder107(_input))?))())?;
let offset_or_data = ((|| PResult::Ok((Decoder107(_input))?))())?;
tiff_main_ifd_fields { tag, r#type, length, offset_or_data }
});
}
accum
}))())?;
let next_ifd_offset = ((|| PResult::Ok((Decoder107(_input))?))())?;
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = ((|| PResult::Ok((Decoder94(_input, header.ident.data.clone() == 2u8, header.ident.class.clone(), header.phnum.clone()))?))())?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = ((|| PResult::Ok((Decoder95(_input, header.ident.data.clone() == 2u8, header.ident.class.clone(), header.shnum.clone()))?))())?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = ((|| PResult::Ok((Decoder96(_input, shdr.r#type.clone(), match shdr.size.clone() {
...
let table_id = ((|| PResult::Ok((Decoder54(_input))?))())?;
...
let created = ((|| PResult::Ok((Decoder78(_input))?))())?;
let modified = ((|| PResult::Ok((Decoder78(_input))?))())?;
...
let ach_vend_id = ((|| PResult::Ok((Decoder54(_input))?))())?;
...
opentype_gpos_table_lookup_list_link_lookups_link_subtables_link::MarkBasePos(inner)
},

5u16 => {
let inner = (Decoder_opentype_layout_mark_lig_pos(_input))?;
opentype_gpos_table_lookup_list_link_lookups_link_subtables_link::MarkLigPos(inner)
},
...
accum.push({
let script_tag = ((|| PResult::Ok((Decoder54(_input))?))())?;
...
accum.push({
let feature_tag = ((|| PResult::Ok((Decoder54(_input))?))())?;
...
fn Decoder54<'input>(_input: &mut Parser<'input>) -> Result<u32, ParseError> {
...
accum.push({
let lang_sys_tag = ((|| PResult::Ok((Decoder54(_input))?))())?;
...
let value_record = ((|| PResult::Ok((Decoder68(_input, table_start.clone(), value_format.clone()))?))())?;
...
accum.push((Decoder68(_input, table_start.clone(), value_format.clone()))?);
}
accum
}))())?;
...
Some((Decoder68(_input, table_start.clone(), value_format2.clone()))?)
} else {
None
}))())?;
...
accum.push({
let value_record1 = ((|| PResult::Ok(if value_format1.x_placement.clone() || value_format1.y_placement.clone() || value_format1.x_advance.clone() || value_format1.y_advance.clone() || value_format1.x_placement_device.clone() || value_format1.y_placement_device.clone() || value_format1.x_advance_device.clone() || value_format1.y_advance_device.clone() {
Some((Decoder69(_input, table_start.clone(), value_format1.clone()))?)
} else {
None
}))())?;
let value_record2 = ((|| PResult::Ok(if value_format2.x_placement.clone() || value_format2.y_placement.clone() || value_format2.x_advance.clone() || value_format2.y_advance.clone() || value_format2.x_placement_device.clone() || value_format2.y_placement_device.clone() || value_format2.x_advance_device.clone() || value_format2.y_advance_device.clone() {
Some((Decoder70(_input, table_start.clone(), value_format2.clone()))?)
} else {
None
}))())?;
...
opentype_layout_mark_lig_pos_ligature_array_offset_link_ligature_attach_offsets_link { component_count, component_records }
}))())?;
_input.close_peek_context()?;
Some(ret)
} else {
...
fn Decoder68<'input>(_input: &mut Parser<'input>, table_start: u32, flags: opentype_common_value_format_flags) -> Result<opentype_common_value_record, ParseError> {
let x_placement = ((|| PResult::Ok(if flags.x_placement.clone() {
Some((Decoder23(_input))?)
} else {
None
...
fn Decoder69<'input>(_input: &mut Parser<'input>, table_start: u32, flags: opentype_common_value_format_flags) -> Result<opentype_common_value_record, ParseError> {
let x_placement = ((|| PResult::Ok(if flags.x_placement.clone() {
Some((Decoder23(_input))?)
} else {
None
...
fn Decoder70<'input>(_input: &mut Parser<'input>, table_start: u32, flags: opentype_common_value_format_flags) -> Result<opentype_common_value_record, ParseError> {
let x_placement = ((|| PResult::Ok(if flags.x_placement.clone() {
Some((Decoder23(_input))?)
} else {
None
...
fn Decoder78<'input>(_input: &mut Parser<'input>) -> Result<u64, ParseError> {
PResult::Ok((Decoder79(_input))?)
}

fn Decoder79<'input>(_input: &mut Parser<'input>) -> Result<u64, ParseError> {
let inner = {
let field0 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field2 = ((|| PResult::Ok((Decoder24(_input))?))())?;
...
13u16 => {
let inner = (Decoder89(_input, _platform.clone()))?;
...
fn Decoder89<'input>(_input: &mut Parser<'input>, _platform: u16) -> Result<opentype_cmap_subtable_format13, ParseError> {
let length = {
_input.open_peek_context();
let ret = ((|| PResult::Ok({
let _ = {
...
let r#type = ((|| PResult::Ok((Decoder115(_input, ident.data.clone() == 2u8))?))())?;
let machine = ((|| PResult::Ok((Decoder116(_input, ident.data.clone() == 2u8))?))())?;
let version = ((|| PResult::Ok((Decoder117(_input, ident.data.clone() == 2u8))?))())?;
...
let flags = ((|| PResult::Ok((Decoder98(_input, ident.data.clone() == 2u8))?))())?;
let ehsize = ((|| PResult::Ok((Decoder118(_input, ident.data.clone() == 2u8))?))())?;
let phentsize = ((|| PResult::Ok((Decoder118(_input, ident.data.clone() == 2u8))?))())?;
let phnum = ((|| PResult::Ok((Decoder118(_input, ident.data.clone() == 2u8))?))())?;
let shentsize = ((|| PResult::Ok((Decoder118(_input, ident.data.clone() == 2u8))?))())?;
let shnum = ((|| PResult::Ok((Decoder118(_input, ident.data.clone() == 2u8))?))())?;
let shstrndx = ((|| PResult::Ok((Decoder118(_input, ident.data.clone() == 2u8))?))())?;
...
fn Decoder94<'input>(_input: &mut Parser<'input>, is_be: bool, class: u8, phnum: u16) -> Result<Vec<elf_phdr_table>, ParseError> {
let mut accum = Vec::new();
for _ in 0..phnum {
...
fn Decoder95<'input>(_input: &mut Parser<'input>, is_be: bool, class: u8, shnum: u16) -> Result<Vec<elf_shdr_table>, ParseError> {
let mut accum = Vec::new();
for _ in 0..shnum {
...
fn Decoder96<'input>(_input: &mut Parser<'input>, r#type: u32, size: u64) -> Result<Vec<u8>, ParseError> {
...
let name = ((|| PResult::Ok((Decoder98(_input, is_be.clone()))?))())?;
let r#type = ((|| PResult::Ok((Decoder99(_input, is_be.clone()))?))())?;
...
let link = ((|| PResult::Ok((Decoder98(_input, is_be.clone()))?))())?;
let info = ((|| PResult::Ok((Decoder103(_input, is_be.clone()))?))())?;
...
2u8 => {
let inner = (Decoder109(_input, is_be.clone()))?;
...
2u8 => {
let inner = (Decoder105(_input, is_be.clone()))?;
...
PResult::Ok((Decoder98(_input, is_be.clone()))?)
}

fn Decoder104<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u32, ParseError> {
let _ = _input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
(Decoder20(_input))?
...
fn Decoder105<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u64, ParseError> {
...
fn Decoder106<'input>(_input: &mut Parser<'input>) -> Result<u64, ParseError> {
let inner = {
let field0 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field2 = ((|| PResult::Ok((Decoder24(_input))?))())?;
...
fn Decoder107<'input>(_input: &mut Parser<'input>) -> Result<u32, ParseError> {
let inner = {
let field0 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field2 = ((|| PResult::Ok((Decoder24(_input))?))())?;
...
fn Decoder108<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u32, ParseError> {
let _ = _input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
(Decoder20(_input))?
...
fn Decoder109<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u64, ParseError> {
let _ = _input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
(Decoder79(_input))?
...
fn Decoder110<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u64, ParseError> {
let _ = _input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
(Decoder79(_input))?
...
let r#type = ((|| PResult::Ok((Decoder98(_input, is_be.clone()))?))())?;
let flags64 = ((|| PResult::Ok((Decoder112(_input, is_be.clone(), class.clone()))?))())?;
let offset = ((|| PResult::Ok((Decoder_elf_types_elf_off(_input, is_be.clone(), class.clone()))?))())?;
...
let flags32 = ((|| PResult::Ok((Decoder113(_input, is_be.clone(), class.clone()))?))())?;
...
fn Decoder112<'input>(_input: &mut Parser<'input>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
PResult::Ok(if class == 2u8 {
Some((Decoder98(_input, is_be.clone()))?)
} else {
None
})
}

fn Decoder113<'input>(_input: &mut Parser<'input>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
...
let class = ((|| PResult::Ok((Decoder120(_input))?))())?;
let data = ((|| PResult::Ok((Decoder121(_input))?))())?;
let version = ((|| PResult::Ok((Decoder122(_input))?))())?;
let os_abi = ((|| PResult::Ok((Decoder123(_input))?))())?;
let abi_version = ((|| PResult::Ok((Decoder124(_input))?))())?;
...
fn Decoder115<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u16, ParseError> {
...
fn Decoder116<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u16, ParseError> {
PResult::Ok((Decoder118(_input, is_be.clone()))?)
}
...
fn Decoder118<'input>(_input: &mut Parser<'input>, is_be: bool) -> Result<u16, ParseError> {
...
fn Decoder119<'input>(_input: &mut Parser<'input>) -> Result<u16, ParseError> {
let inner = {
let field0 = ((|| PResult::Ok((Decoder24(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder24(_input))?))())?;
(field0, field1)
...
fn Decoder121<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let inner = (Decoder24(_input))?;
PResult::Ok(if ((|x: u8| PResult::Ok(x <= 2u8))(inner.clone()))? {
inner
} else {
...
fn Decoder122<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let inner = (Decoder24(_input))?;
...
fn Decoder123<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
PResult::Ok((Decoder24(_input))?)
}

fn Decoder124<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
PResult::Ok((Decoder24(_input))?)
}
...
let __nul_or_wsp = ((|| PResult::Ok((Decoder129(_input))?))())?;
let __padding = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
let __nil = ((|| PResult::Ok((Decoder129(_input))?))())?;
...
let typeflag = ((|| PResult::Ok((Decoder130(_input))?))())?;
...
fn Decoder129<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
...
fn Decoder130<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(b)
}

fn Decoder131<'input>(_input: &mut Parser<'input>) -> Result<tar_ascii_string_opt0, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
let field0 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let field2 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let field3 = ((|| PResult::Ok((Decoder130(_input))?))())?;
...
fn Decoder136<'input>(_input: &mut Parser<'input>) -> Result<(u8, u8, u8, u8, u8, u8, u8, u8), ParseError> {
let field0 = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 137 {
b
...
let tag = ((|| PResult::Ok((Decoder183(_input))?))())?;
let data = ((|| PResult::Ok({
let sz = length as usize<>;
_input.start_slice(sz)?;
...
let tag = ((|| PResult::Ok((Decoder152(_input))?))())?;
let data = ((|| PResult::Ok({
let sz = length as usize<>;
_input.start_slice(sz)?;
let ret = ((|| PResult::Ok((Decoder153(_input))?))())?;
_input.end_slice()?;
ret
}))())?;
let crc = ((|| PResult::Ok((Decoder20(_input))?))())?;
...
fn Decoder143<'input>(_input: &mut Parser<'input>) -> Result<(), ParseError> {
PResult::Ok(())
}
...
let r#final = ((|| PResult::Ok((Decoder146(_input))?))())?;
let r#type = ((|| PResult::Ok({
let inner = {
let field0 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder146(_input))?))())?;
(field0, field1)
};
...
fn Decoder146<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(b)
}
...
let field0 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field2 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field3 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field4 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field5 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field6 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field7 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field8 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field9 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field10 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field11 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field12 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field13 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field14 = ((|| PResult::Ok((Decoder146(_input))?))())?;
let field15 = ((|| PResult::Ok((Decoder146(_input))?))())?;
...
0u8 => {
(Decoder151(_input, 0u8, 1u16))?
},

1u8 => {
(Decoder151(_input, 0u8, 2u16))?
},

2u8 => {
(Decoder151(_input, 0u8, 3u16))?
},

3u8 => {
(Decoder151(_input, 0u8, 4u16))?
},

4u8 => {
(Decoder151(_input, 1u8, 5u16))?
},

5u8 => {
(Decoder151(_input, 1u8, 7u16))?
},

6u8 => {
(Decoder151(_input, 2u8, 9u16))?
},

7u8 => {
(Decoder151(_input, 2u8, 13u16))?
},

8u8 => {
(Decoder151(_input, 3u8, 17u16))?
},

9u8 => {
(Decoder151(_input, 3u8, 25u16))?
},

10u8 => {
(Decoder151(_input, 4u8, 33u16))?
},

11u8 => {
(Decoder151(_input, 4u8, 49u16))?
},

12u8 => {
(Decoder151(_input, 5u8, 65u16))?
},

13u8 => {
(Decoder151(_input, 5u8, 97u16))?
},

14u8 => {
(Decoder151(_input, 6u8, 129u16))?
},

15u8 => {
(Decoder151(_input, 6u8, 193u16))?
},

16u8 => {
(Decoder151(_input, 7u8, 257u16))?
},

17u8 => {
(Decoder151(_input, 7u8, 385u16))?
},

18u8 => {
(Decoder151(_input, 8u8, 513u16))?
},

19u8 => {
(Decoder151(_input, 8u8, 769u16))?
},

20u8 => {
(Decoder151(_input, 9u8, 1025u16))?
},

21u8 => {
(Decoder151(_input, 9u8, 1537u16))?
},

22u8 => {
(Decoder151(_input, 10u8, 2049u16))?
},

23u8 => {
(Decoder151(_input, 10u8, 3073u16))?
},

24u8 => {
(Decoder151(_input, 11u8, 4097u16))?
},

25u8 => {
(Decoder151(_input, 11u8, 6145u16))?
},

26u8 => {
(Decoder151(_input, 12u8, 8193u16))?
},

27u8 => {
(Decoder151(_input, 12u8, 12289u16))?
},

28u8 => {
(Decoder151(_input, 13u8, 16385u16))?
},

29u8 => {
(Decoder151(_input, 13u8, 24577u16))?
},
...
fn Decoder151<'input>(_input: &mut Parser<'input>, extra_bits: u8, start: u16) -> Result<deflate_distance_record, ParseError> {
...
fn Decoder154<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
...
fn Decoder155<'input>(_input: &mut Parser<'input>) -> Result<Vec<png_plte>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let inner = {
let field0 = ((|| PResult::Ok((Decoder181(_input))?))())?;
let field1 = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 0 {
b
...
let compressed_profile = ((|| PResult::Ok((Decoder182(_input))?))())?;
...
let inner = {
let field0 = ((|| PResult::Ok((Decoder176(_input))?))())?;
let field1 = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 0 {
b
...
let inner = {
let field0 = ((|| PResult::Ok((Decoder178(_input))?))())?;
let field1 = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 0 {
b
...
let keyword = ((|| PResult::Ok({
let inner = {
let field0 = ((|| PResult::Ok((Decoder175(_input))?))())?;
let field1 = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 0 {
b
...
if matching_ix == 0 {
let next_elem = (Decoder130(_input))?;
accum.push(next_elem);
} else {
break
}
...
return Err(ParseError::FalsifiedWhere(16586067667342674799u64));
}
}))())?;
let flags = ((|| PResult::Ok({
let inner = {
...
fn Decoder173<'input>(_input: &mut Parser<'input>) -> Result<Vec<char>, ParseError> {
PResult::Ok((Decoder174(_input))?)
}

fn Decoder174<'input>(_input: &mut Parser<'input>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
fn Decoder175<'input>(_input: &mut Parser<'input>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
fn Decoder176<'input>(_input: &mut Parser<'input>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
fn Decoder178<'input>(_input: &mut Parser<'input>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
fn Decoder179<'input>(_input: &mut Parser<'input>) -> Result<zlib_main, ParseError> {
let compression_method_flags = ((|| PResult::Ok({
let inner = {
let inner = {
let b = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(8414850703985147087u64));
}
}))())?;
let flags = ((|| PResult::Ok({
let inner = {
...
fn Decoder180<'input>(_input: &mut Parser<'input>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
fn Decoder181<'input>(_input: &mut Parser<'input>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
fn Decoder182<'input>(_input: &mut Parser<'input>) -> Result<zlib_main, ParseError> {
let compression_method_flags = ((|| PResult::Ok({
let inner = {
let inner = {
let b = _input.read_byte()?;
...
fn Decoder183<'input>(_input: &mut Parser<'input>) -> Result<(u8, u8, u8, u8), ParseError> {
let field0 = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 73 {
b
...
let r#type = ((|| PResult::Ok((Decoder186(_input))?))())?;
...
let inner = {
let major_brand = ((|| PResult::Ok((Decoder186(_input))?))())?;
...
if matching_ix == 0 {
let next_elem = (Decoder186(_input))?;
accum.push(next_elem);
} else {
break
}
...
fn Decoder186<'input>(_input: &mut Parser<'input>) -> Result<(u8, u8, u8, u8), ParseError> {
let field0 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let field1 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let field2 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let field3 = ((|| PResult::Ok((Decoder130(_input))?))())?;
...
let handler_type = ((|| PResult::Ok((Decoder186(_input))?))())?;
...
let name = ((|| PResult::Ok((Decoder193(_input))?))())?;
...
1u8 => {
let inner = {
let creation_time = ((|| PResult::Ok((Decoder79(_input))?))())?;
let modification_time = ((|| PResult::Ok((Decoder79(_input))?))())?;
let timescale = ((|| PResult::Ok((Decoder20(_input))?))())?;
let duration = ((|| PResult::Ok((Decoder79(_input))?))())?;
...
let component_subtype = ((|| PResult::Ok((Decoder186(_input))?))())?;
...
let component_name = ((|| PResult::Ok((Decoder193(_input))?))())?;
...
let item_name = ((|| PResult::Ok((Decoder200(_input))?))())?;
let content_type = ((|| PResult::Ok((Decoder201(_input))?))())?;
let content_encoding = ((|| PResult::Ok((Decoder202(_input))?))())?;
...
let item_protection_index = ((|| PResult::Ok((Decoder23(_input))?))())?;
let item_type = ((|| PResult::Ok((Decoder186(_input))?))())?;
let item_name = ((|| PResult::Ok((Decoder203(_input))?))())?;
let extra_fields = ((|| PResult::Ok(match item_type {
(109u8, 105u8, 109u8, 101u8) => {
let inner = {
let content_type = ((|| PResult::Ok((Decoder204(_input))?))())?;
...
let inner = {
let item_uri_type = ((|| PResult::Ok((Decoder204(_input))?))())?;
...
fn Decoder200<'input>(_input: &mut Parser<'input>) -> Result<base_asciiz_string, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
fn Decoder201<'input>(_input: &mut Parser<'input>) -> Result<base_asciiz_string, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
fn Decoder202<'input>(_input: &mut Parser<'input>) -> Result<base_asciiz_string, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
fn Decoder203<'input>(_input: &mut Parser<'input>) -> Result<base_asciiz_string, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
fn Decoder204<'input>(_input: &mut Parser<'input>) -> Result<base_asciiz_string, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
let data = ((|| PResult::Ok((Decoder229(_input))?))())?;
...
fn Decoder214<'input>(_input: &mut Parser<'input>) -> Result<jpeg_scan, ParseError> {
let segments = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
fn Decoder217<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let tree_index = {
_input.open_peek_context();
let b = _input.read_byte()?;
{
...
fn Decoder218<'input>(_input: &mut Parser<'input>) -> Result<jpeg_eoi, ParseError> {
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
b
...
fn Decoder221<'input>(_input: &mut Parser<'input>) -> Result<jpeg_eoi, ParseError> {
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
b
...
fn Decoder222<'input>(_input: &mut Parser<'input>) -> Result<jpeg_eoi, ParseError> {
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
b
...
fn Decoder223<'input>(_input: &mut Parser<'input>) -> Result<jpeg_eoi, ParseError> {
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
b
...
fn Decoder224<'input>(_input: &mut Parser<'input>) -> Result<jpeg_eoi, ParseError> {
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
b
...
fn Decoder225<'input>(_input: &mut Parser<'input>) -> Result<jpeg_eoi, ParseError> {
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
b
...
return Err(ParseError::FalsifiedWhere(2750471357152633230u64));
}
}))())?;
PResult::Ok(jpeg_dnl_data { num_lines })
}

fn Decoder229<'input>(_input: &mut Parser<'input>) -> Result<jpeg_scan_data, ParseError> {
let scan_data = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
fn Decoder233<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder234<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder235<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder236<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder237<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder238<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder239<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder240<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder241<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder242<'input>(_input: &mut Parser<'input>) -> Result<jpeg_sof15, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(2157707350523277837u64));
}
}))())?;
let image_components = ((|| PResult::Ok({
let mut accum = Vec::new();
...
return Err(ParseError::FalsifiedWhere(15134222038433106385u64));
}
}))())?;
...
fn Decoder254<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder255<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder256<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder257<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder258<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder259<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder260<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
fn Decoder261<'input>(_input: &mut Parser<'input>) -> Result<jpeg_com, ParseError> {
let marker = ((|| PResult::Ok({
let ff = ((|| PResult::Ok({
let b = _input.read_byte()?;
if b == 255 {
...
let identifier = ((|| PResult::Ok((Decoder269(_input))?))())?;
...
let identifier = ((|| PResult::Ok((Decoder273(_input))?))())?;
let data = ((|| PResult::Ok(match identifier.string.as_slice() {
[74u8, 70u8, 73u8, 70u8] => {
...
let timestamp = ((|| PResult::Ok((Decoder107(_input))?))())?;
...
let xlen = ((|| PResult::Ok((Decoder119(_input))?))())?;
...
let crc = ((|| PResult::Ok((Decoder107(_input))?))())?;
let length = ((|| PResult::Ok((Decoder107(_input))?))())?;
...
fn Decoder283<'input>(_input: &mut Parser<'input>) -> Result<base_asciiz_string, ParseError> {
let string = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
let si1 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let si2 = ((|| PResult::Ok((Decoder130(_input))?))())?;
let len = ((|| PResult::Ok((Decoder119(_input))?))())?;
let data = ((|| PResult::Ok({
let mut accum = Vec::new();
for _ in 0..len {
accum.push((Decoder24(_input))?);
...
fn Decoder285<'input>(_input: &mut Parser<'input>) -> Result<Vec<gzip_main>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
PResult::Ok((Decoder289(_input))?)
}

fn Decoder287<'input>(_input: &mut Parser<'input>) -> Result<gzip_fcomment, ParseError> {
let comment = ((|| PResult::Ok((Decoder288(_input))?))())?;
PResult::Ok(gzip_fcomment { comment })
}
...
let terminator = ((|| PResult::Ok((Decoder299(_input))?))())?;
...
fn Decoder299<'input>(_input: &mut Parser<'input>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(if b == 0 {
b
} else {
...
let delay_time = ((|| PResult::Ok((Decoder119(_input))?))())?;
...
let text_grid_left_position = ((|| PResult::Ok((Decoder119(_input))?))())?;
let text_grid_top_position = ((|| PResult::Ok((Decoder119(_input))?))())?;
let text_grid_width = ((|| PResult::Ok((Decoder119(_input))?))())?;
let text_grid_height = ((|| PResult::Ok((Decoder119(_input))?))())?;
...
let image_left_position = ((|| PResult::Ok((Decoder119(_input))?))())?;
let image_top_position = ((|| PResult::Ok((Decoder119(_input))?))())?;
let image_width = ((|| PResult::Ok((Decoder119(_input))?))())?;
let image_height = ((|| PResult::Ok((Decoder119(_input))?))())?;
let flags = ((|| PResult::Ok({
let inner = {
let b = _input.read_byte()?;
b
...
fn Decoder305<'input>(_input: &mut Parser<'input>) -> Result<png_plte, ParseError> {
let r = ((|| PResult::Ok((Decoder24(_input))?))())?;
let g = ((|| PResult::Ok((Decoder24(_input))?))())?;
let b = ((|| PResult::Ok((Decoder24(_input))?))())?;
PResult::Ok(png_plte { r, g, b })
...
let screen_width = ((|| PResult::Ok((Decoder119(_input))?))())?;
let screen_height = ((|| PResult::Ok((Decoder119(_input))?))())?;
let flags = ((|| PResult::Ok({
let inner = {
let b = _input.read_byte()?;
b
...
fn Decoder308<'input>(_input: &mut Parser<'input>) -> Result<u32, ParseError> {
let inner = {
let field0 = ((|| PResult::Ok({
let mut accum = Vec::new();
while _input.remaining() > 0 {
