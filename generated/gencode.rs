...
/// expected size: 3
/// trait-unready: multiple (2) decoders exist (d#{431, 466})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{442, 450})
#[derive(Debug, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (2) decoders exist (d#{269, 270})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 1
/// trait-unready: multiple (10) decoders exist (d#{361, 363, 374, 375, 376, 377, 378, 379, 380, 381})
#[derive(Debug, Copy, Clone)]
pub struct jpeg_soi {
marker: u8
}
...
/// expected size: 72
/// trait-ready: unique decoder function (d#428)
#[derive(Debug, Clone)]
...
/// expected size: 56
/// trait-unready: multiple (3) decoders exist (d#{251, 252, 253})
#[derive(Debug, Clone)]
...
/// expected size: 80
/// trait-ready: unique decoder function (d#426)
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-ready: unique decoder function (d#427)
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (16) decoders exist (d#{409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 419, 420, 421, 422, 423, 479})
#[derive(Debug, Clone)]
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{473, 475})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (3) decoders exist (d#{402, 474, 477})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (3) decoders exist (d#{401, 476, 481})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (14) decoders exist (d#{388, 389, 390, 391, 392, 393, 394, 395, 396, 397, 398, 399, 400, 480})
#[derive(Debug, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{371, 387})
#[derive(Debug, Clone)]
...
/// expected size: 112
/// trait-unready: multiple (2) decoders exist (d#{367, 369})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{336, 338})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{210, 211})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (6) decoders exist (d#{164, 165, 168, 169, 172, 173})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 96
/// trait-unready: multiple (4) decoders exist (d#{260, 291, 297, 302})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (3) decoders exist (d#{246, 248, 249})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{12, 448})
#[derive(Debug, Clone)]
pub struct tar_main {
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{478, 483})
#[derive(Debug, Copy, Clone)]
...
let next_elem = (Decoder472(_input))?;
accum.push(next_elem)
}
};
PResult::Ok(accum)
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder216(_input, header.ident.data == 2u8, header.ident.class, header.phnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder217(_input, header.ident.data == 2u8, header.ident.class, header.shnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder218(_input, shdr.r#type, match shdr.size {
...
let created = (Decoder201(_input))?;
let modified = (Decoder201(_input))?;
let glyph_extents = {
let x_min = {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
...
PResult::Ok(opentype_svg_document_record { start_glyph_id, end_glyph_id, svg_document_offset, svg_document_length, _svg_document })
}
...
let mut accum = Vec::new();
for _ in 0..try_sub!(region_index_count, word_delta_count.word_count, 18270091135093349626u64) {
let next_elem = {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let delta_data_half_word = {
let mut accum = Vec::new();
for _ in 0..try_sub!(region_index_count, word_delta_count.word_count, 6185506036438099345u64) {
let next_elem = {
let raw = _input.read_byte()?;
raw as i8
};
...
/// d#85
fn Decoder85(_input: &mut Parser<'_>) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
(Decoder86(_input, point_count as u16))?
},
...
(Decoder86(_input, ((hi as u16) & 127u16) << 8u16 | (lo as u16)))?
},
...
/// d#86
fn Decoder86(_input: &mut Parser<'_>, point_count: u16) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(8958899994948144829u64));
inner
}
};
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(15803403730818557393u64));
inner
}
};
let minor_version = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(8390724546948265409u64));
inner
}
};
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(4251627061094365437u64));
inner
}
};
...
_ => {
return Err(ParseError::FailToken(1278184758971178969u64));
}
};
...
_ => {
return Err(ParseError::FailToken(3426398976290336157u64));
}
};
...
_ => {
return Err(ParseError::FailToken(11250208753083412758u64));
}
};
...
_ => {
return Err(ParseError::FailToken(2153064741293804702u64));
}
};
...
_ => {
return Err(ParseError::FailToken(1588651938759015246u64));
}
};
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(7744051144774795087u64));
inner
}
};
...
_ => {
return Err(ParseError::FailToken(9798710097031164942u64));
}
})
}
...
_ => {
return Err(ParseError::FailToken(9819345728844658158u64));
}
};
...
1u16 => {
match ((succ(try_sub!(end_size, start_size, 18147521187885925800u64))) / 8u16) * 8u16 < (succ(try_sub!(end_size, start_size, 7364705619221056123u64))) {
true => {
succ((succ(try_sub!(end_size, start_size, 2404222719611925354u64))) / 8u16)
},

false => {
(succ(try_sub!(end_size, start_size, 13091357170910775568u64))) / 8u16
}
}
},

2u16 => {
match ((succ(try_sub!(end_size, start_size, 17170585774888887431u64))) / 4u16) * 4u16 < (succ(try_sub!(end_size, start_size, 1548601315919054830u64))) {
true => {
succ((succ(try_sub!(end_size, start_size, 9339905250237811640u64))) / 4u16)
},

false => {
(succ(try_sub!(end_size, start_size, 6039633234730737119u64))) / 4u16
}
}
},

3u16 => {
match ((succ(try_sub!(end_size, start_size, 1654541969082323602u64))) / 2u16) * 2u16 < (succ(try_sub!(end_size, start_size, 1272171534487716374u64))) {
true => {
succ((succ(try_sub!(end_size, start_size, 7000022209644146403u64))) / 2u16)
},

false => {
(succ(try_sub!(end_size, start_size, 8451221290566481190u64))) / 2u16
}
}
},
...
Some((Decoder165(_input, value_format2, table_view))?)
} else {
None
};
...
/// d#165
fn Decoder165<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record1 = if value_format1.x_placement || value_format1.y_placement || value_format1.x_advance || value_format1.y_advance || value_format1.x_placement_device || value_format1.y_placement_device || value_format1.x_advance_device || value_format1.y_advance_device {
Some((Decoder168(_input, value_format1, set_view))?)
} else {
None
};
let value_record2 = if value_format2.x_placement || value_format2.y_placement || value_format2.x_advance || value_format2.y_advance || value_format2.x_placement_device || value_format2.y_placement_device || value_format2.x_advance_device || value_format2.y_advance_device {
Some((Decoder169(_input, value_format2, set_view))?)
} else {
None
};
...
/// d#168
fn Decoder168<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#169
fn Decoder169<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record = (Decoder173(_input, value_format, table_view))?;
...
let next_elem = (Decoder172(_input, value_format, table_view))?;
accum.push(next_elem)
};
accum
};
...
/// d#172
fn Decoder172<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#173
fn Decoder173<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(10875553067752207222u64));
inner
}
};
...
/// d#201
fn Decoder201(_input: &mut Parser<'_>) -> Result<i64, ParseError> {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u64be(x)
};
...
13u16 => {
let inner = (Decoder211(_input, _platform))?;
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(9179996462972575343u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(3675496117133668659u64));
inner
}
};
let inner = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(8327471529801851430u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(16529910322175208638u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(12187643960709778443u64));
inner
}
};
let start_code = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(8766708729375264031u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(1369437808023015077u64));
inner
}
};
{
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(163858356033350300u64));
inner
}
};
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(12530712830475607577u64));
inner
}
};
let length = {
...
/// d#211
fn Decoder211(_input: &mut Parser<'_>, _platform: u16) -> Result<opentype_cmap_subtable_format12, ParseError> {
let length = {
_input.open_peek_context();
let ret = ((|| {
let _format = {
...
let r#type = (Decoder235(_input, ident.data == 2u8))?;
let machine = (Decoder236(_input, ident.data == 2u8))?;
let version = (Decoder237(_input, ident.data == 2u8))?;
...
let flags = (Decoder220(_input, ident.data == 2u8))?;
let ehsize = (Decoder238(_input, ident.data == 2u8))?;
let phentsize = (Decoder238(_input, ident.data == 2u8))?;
let phnum = (Decoder238(_input, ident.data == 2u8))?;
let shentsize = (Decoder238(_input, ident.data == 2u8))?;
let shnum = (Decoder238(_input, ident.data == 2u8))?;
let shstrndx = (Decoder238(_input, ident.data == 2u8))?;
...
/// d#216
fn Decoder216(_input: &mut Parser<'_>, is_be: bool, class: u8, phnum: u16) -> Result<Vec<elf_phdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..phnum {
...
/// d#217
fn Decoder217(_input: &mut Parser<'_>, is_be: bool, class: u8, shnum: u16) -> Result<Vec<elf_shdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..shnum {
...
/// d#218
fn Decoder218(_input: &mut Parser<'_>, r#type: u32, size: u64) -> Result<Vec<u8>, ParseError> {
PResult::Ok({
let _ = r#type;
let mut accum = Vec::new();
for _ in 0..size {
...
let name = (Decoder220(_input, is_be))?;
let r#type = (Decoder221(_input, is_be))?;
...
let link = (Decoder220(_input, is_be))?;
let info = (Decoder225(_input, is_be))?;
...
/// d#220
fn Decoder220(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
...
/// d#221
fn Decoder221(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
let inner = (Decoder220(_input, is_be))?;
let is_valid = {
let sh_type = inner;
...
2u8 => {
let inner = (Decoder230(_input, is_be))?;
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder228(_input, is_be))?;
elf_types_elf_addr::Addr32(inner)
},

2u8 => {
let inner = (Decoder229(_input, is_be))?;
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder226(_input, is_be))?;
elf_types_elf_off::Off32(inner)
},

2u8 => {
let inner = (Decoder227(_input, is_be))?;
...
/// d#225
fn Decoder225(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
Decoder220(_input, is_be)
}

/// d#226
fn Decoder226(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#227
fn Decoder227(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#228
fn Decoder228(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#229
fn Decoder229(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#230
fn Decoder230(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
let r#type = (Decoder220(_input, is_be))?;
let flags64 = (Decoder232(_input, is_be, class))?;
let offset = (Decoder_elf_types_elf_off(_input, is_be, class))?;
...
let flags32 = (Decoder233(_input, is_be, class))?;
...
/// d#232
fn Decoder232(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
PResult::Ok(if class == 2u8 {
Some((Decoder220(_input, is_be))?)
} else {
None
})
}
...
/// d#233
fn Decoder233(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
...
let class = (Decoder239(_input))?;
let data = (Decoder240(_input))?;
let version = (Decoder241(_input))?;
let os_abi = (Decoder242(_input))?;
let abi_version = (Decoder243(_input))?;
{
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#235
fn Decoder235(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
let inner = (Decoder238(_input, is_be))?;
let is_valid = {
let r#type = inner;
...
/// d#236
fn Decoder236(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
Decoder238(_input, is_be)
}

/// d#237
fn Decoder237(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
let inner = (Decoder220(_input, is_be))?;
let is_valid = {
let x = inner;
x <= 1u32
...
/// d#238
fn Decoder238(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
_input.skip_align(2)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?);
...
/// d#239
fn Decoder239(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
/// d#240
fn Decoder240(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
/// d#241
fn Decoder241(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 1u8
...
/// d#242
fn Decoder242(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
_input.read_byte()
}

/// d#243
fn Decoder243(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
_input.read_byte()
}
...
try_sub!(bit as u8, 48u8, 4649034608147552416u64)
};
let _o7 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 16299205781335471965u64)
};
let _o6 = {
let bit = {
let b = _input.read_byte()?;
...
/// d#247
fn Decoder247(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
...
/// d#248
fn Decoder248(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#249
fn Decoder249(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#252
fn Decoder252(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16le(x)
};
...
/// d#253
fn Decoder253(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#255
fn Decoder255(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
...
return Err(ParseError::FalsifiedWhere(30874382969105279u64));
}
};
let tag = (Decoder273(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(9791114990321288281u64));
}
};
let tag = (Decoder271(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
let ret = (Decoder272(_input))?;
_input.end_slice()?;
ret
};
let crc = {
...
return Err(ParseError::FalsifiedWhere(1595897747104696027u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(2481175643332430741u64));
}
};
let tag = (Decoder262(_input))?;
let crc = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
/// d#262
fn Decoder262(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let literal_length_distance_alphabet_code_lengths = (Decoder268(_input, hlit, hdist, &code_length_alphabet_code_lengths))?;
...
/// d#268
fn Decoder268(_input: &mut Parser<'_>, hlit: u8, hdist: u8, code_length_alphabet_code_lengths: &[u8]) -> Result<Vec<deflate_dynamic_huffman_literal_length_distance_alphabet_code_lengths>, ParseError> {
...
0u8 => {
(Decoder270(_input, 0u8, 1u16))?
},

1u8 => {
(Decoder270(_input, 0u8, 2u16))?
},

2u8 => {
(Decoder270(_input, 0u8, 3u16))?
},

3u8 => {
(Decoder270(_input, 0u8, 4u16))?
},

4u8 => {
(Decoder270(_input, 1u8, 5u16))?
},

5u8 => {
(Decoder270(_input, 1u8, 7u16))?
},

6u8 => {
(Decoder270(_input, 2u8, 9u16))?
},

7u8 => {
(Decoder270(_input, 2u8, 13u16))?
},

8u8 => {
(Decoder270(_input, 3u8, 17u16))?
},

9u8 => {
(Decoder270(_input, 3u8, 25u16))?
},

10u8 => {
(Decoder270(_input, 4u8, 33u16))?
},

11u8 => {
(Decoder270(_input, 4u8, 49u16))?
},

12u8 => {
(Decoder270(_input, 5u8, 65u16))?
},

13u8 => {
(Decoder270(_input, 5u8, 97u16))?
},

14u8 => {
(Decoder270(_input, 6u8, 129u16))?
},

15u8 => {
(Decoder270(_input, 6u8, 193u16))?
},

16u8 => {
(Decoder270(_input, 7u8, 257u16))?
},

17u8 => {
(Decoder270(_input, 7u8, 385u16))?
},

18u8 => {
(Decoder270(_input, 8u8, 513u16))?
},

19u8 => {
(Decoder270(_input, 8u8, 769u16))?
},

20u8 => {
(Decoder270(_input, 9u8, 1025u16))?
},

21u8 => {
(Decoder270(_input, 9u8, 1537u16))?
},

22u8 => {
(Decoder270(_input, 10u8, 2049u16))?
},

23u8 => {
(Decoder270(_input, 10u8, 3073u16))?
},

24u8 => {
(Decoder270(_input, 11u8, 4097u16))?
},

25u8 => {
(Decoder270(_input, 11u8, 6145u16))?
},

26u8 => {
(Decoder270(_input, 12u8, 8193u16))?
},

27u8 => {
(Decoder270(_input, 12u8, 12289u16))?
},

28u8 => {
(Decoder270(_input, 13u8, 16385u16))?
},

29u8 => {
(Decoder270(_input, 13u8, 24577u16))?
},

30u8..=31u8 => {
return Err(ParseError::FailToken(11225936372640404826u64));
},
...
/// d#270
fn Decoder270(_input: &mut Parser<'_>, extra_bits: u8, start: u16) -> Result<deflate_distance_record0, ParseError> {
...
/// d#271
fn Decoder271(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
/// d#274
fn Decoder274(_input: &mut Parser<'_>) -> Result<Vec<gif_color_table_entry>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let compressed_profile = (Decoder302(_input))?;
...
return Err(ParseError::FalsifiedWhere(9452754313802575046u64));
}
};
...
let compressed_text = {
let zlib = (Decoder291(_input))?;
let mut buf_parser = Parser::new(slice_all(&zlib.data.inflate));
let buf_input = &mut buf_parser;
(Decoder292(buf_input))?
};
...
/// d#289
fn Decoder289(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#290
fn Decoder290(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#291
fn Decoder291(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(9730514595131843432u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#292
fn Decoder292(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
Decoder293(_input)
}

/// d#293
fn Decoder293(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#295
fn Decoder295(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#296
fn Decoder296(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#297
fn Decoder297(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(17190949120592669315u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#298
fn Decoder298(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#299
fn Decoder299(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
Decoder300(_input)
}

/// d#300
fn Decoder300(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#301
fn Decoder301(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#302
fn Decoder302(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
/// d#303
fn Decoder303(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let major_brand = (Decoder306(_input))?;
let minor_version = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
if matching_ix == 0 {
let next_elem = (Decoder306(_input))?;
accum.push(next_elem)
} else {
break
}
...
try_sub!(x, 16u64, 17636172564439370608u64)
},
...
let component_subtype = (Decoder306(_input))?;
...
/// d#338
fn Decoder338(_input: &mut Parser<'_>) -> Result<mpeg4_dref_data_data, ParseError> {
let size_field = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
let handler_type = (Decoder306(_input))?;
let reserved = {
let arg0 = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
...
try_sub!(x, 16u64, 11570281271401624317u64)
},
...
try_sub!(x, 16u64, 1378805635639824117u64)
},
...
let item_type = (Decoder306(_input))?;
let item_name = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
if matching_ix == 0 {
let next_elem = (Decoder369(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#363
fn Decoder363(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
15 => {
let inner = (Decoder418(_input))?;
jpeg_table_or_misc::app11(inner)
},

16 => {
let inner = (Decoder419(_input))?;
jpeg_table_or_misc::app12(inner)
},

17 => {
let inner = (Decoder420(_input))?;
...
/// d#369
fn Decoder369(_input: &mut Parser<'_>) -> Result<jpeg_scan, ParseError> {
let segments = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#373
fn Decoder373(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
/// d#375
fn Decoder375(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#376
fn Decoder376(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#377
fn Decoder377(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#378
fn Decoder378(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#379
fn Decoder379(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#381
fn Decoder381(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
return Err(ParseError::FalsifiedWhere(17055268834995250246u64));
})
}
...
/// d#387
fn Decoder387(_input: &mut Parser<'_>) -> Result<jpeg_scan_data, ParseError> {
let scan_data = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#389
fn Decoder389(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#390
fn Decoder390(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#391
fn Decoder391(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#392
fn Decoder392(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#393
fn Decoder393(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#394
fn Decoder394(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#395
fn Decoder395(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#396
fn Decoder396(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#397
fn Decoder397(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#398
fn Decoder398(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#399
fn Decoder399(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#410
fn Decoder410(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#411
fn Decoder411(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#412
fn Decoder412(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#413
fn Decoder413(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#414
fn Decoder414(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#415
fn Decoder415(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#416
fn Decoder416(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#417
fn Decoder417(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#418
fn Decoder418(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#419
fn Decoder419(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#420
fn Decoder420(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(1016564408906296566u64));
})
}
...
/// d#441
fn Decoder441(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#447
fn Decoder447(_input: &mut Parser<'_>) -> Result<Vec<gzip_main>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#448
fn Decoder448(_input: &mut Parser<'_>) -> Result<tar_main, ParseError> {
let contents = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#449
fn Decoder449(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#450
fn Decoder450(_input: &mut Parser<'_>) -> Result<gzip_fcomment, ParseError> {
let comment = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
let next_elem = (Decoder466(_input))?;
accum.push(next_elem)
};
Some(accum)
} else {
...
let terminator = (Decoder460(_input))?;
...
/// d#460
fn Decoder460(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(if b == 0 {
b
} else {
...
/// d#466
fn Decoder466(_input: &mut Parser<'_>) -> Result<gif_color_table_entry, ParseError> {
let r = _input.read_byte()?;
let g = _input.read_byte()?;
let b = _input.read_byte()?;
PResult::Ok(gif_color_table_entry { r, g, b })
...
/// d#472
fn Decoder472(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let tuple_var = {
let arg0 = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
/// d#474
fn Decoder474(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder475(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
if b == 0 {
b
...
/// d#475
fn Decoder475(_input: &mut Parser<'_>) -> Result<jpeg_dhp_image_component_sampling_factor, ParseError> {
let _packed_bits = _input.read_byte()?;
let horizontal = _packed_bits >> 4u8 & 15u8;
let vertical = _packed_bits & 15u8;
PResult::Ok(jpeg_dhp_image_component_sampling_factor { horizontal, vertical })
...
/// d#476
fn Decoder476(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
...
return Err(ParseError::FalsifiedWhere(9966940320814099155u64));
}
};
let num_image_components = {
let inner = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(15519482928753245836u64));
}
};
let image_components = {
let mut accum = Vec::new();
...
/// d#477
fn Decoder477(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder475(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(2157749958308397018u64));
}
};
...
/// d#479
fn Decoder479(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#480
fn Decoder480(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(11349426438005611188u64));
}
};
let num_image_components = {
let inner = _input.read_byte()?;
...
/// d#483
fn Decoder483(_input: &mut Parser<'_>) -> Result<jpeg_exp_data, ParseError> {
let expand_horizontal_vertical = {
let inner = {
let _packed_bits = _input.read_byte()?;
let expand_horizontal = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(10986984519025233593u64));
}
};
PResult::Ok(jpeg_exp_data { expand_horizontal_vertical })
}
