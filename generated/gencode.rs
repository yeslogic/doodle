...
/// expected size: 3
/// trait-unready: multiple (2) decoders exist (d#{426, 461})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{437, 445})
#[derive(Debug, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (2) decoders exist (d#{264, 265})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 1
/// trait-unready: multiple (10) decoders exist (d#{356, 358, 369, 370, 371, 372, 373, 374, 375, 376})
#[derive(Debug, Copy, Clone)]
pub struct jpeg_soi {
marker: u8
}
...
/// expected size: 56
/// trait-unready: multiple (3) decoders exist (d#{246, 247, 248})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (16) decoders exist (d#{404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 474})
#[derive(Debug, Clone)]
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{468, 470})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (3) decoders exist (d#{397, 469, 472})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (3) decoders exist (d#{396, 471, 476})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (14) decoders exist (d#{383, 384, 385, 386, 387, 388, 389, 390, 391, 392, 393, 394, 395, 475})
#[derive(Debug, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{366, 382})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{331, 333})
#[derive(Debug, Clone)]
...
/// expected size: 28
/// trait-unready: multiple (2) decoders exist (d#{33, 52})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{35, 53})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (6) decoders exist (d#{159, 160, 163, 164, 167, 168})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 96
/// trait-unready: multiple (4) decoders exist (d#{255, 286, 292, 297})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (3) decoders exist (d#{241, 243, 244})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{12, 443})
#[derive(Debug, Clone)]
pub struct tar_main {
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{473, 478})
#[derive(Debug, Copy, Clone)]
...
let next_elem = (Decoder467(_input))?;
accum.push(next_elem)
}
};
PResult::Ok(accum)
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder211(_input, header.ident.data == 2u8, header.ident.class, header.phnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder212(_input, header.ident.data == 2u8, header.ident.class, header.shnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder213(_input, shdr.r#type, match shdr.size {
...
let sz = table.length as usize;
view_input.start_slice(sz)?;
let ret = (Decoder53(view_input, match vhea {
Some(ref x) => {
x
},
...
_input.skip_remainder();
PResult::Ok(opentype_table_directory_table_links { cmap, head, hhea, maxp, hmtx, name, os2, post, cvt, fpgm, loca, glyf, prep, gasp, base, gdef, gpos, gsub, avar, fvar, gvar, hvar, kern, stat, vhea, vmtx, dsig, hdmx, vdmx })
}
...
let created = (Decoder196(_input))?;
let modified = (Decoder196(_input))?;
let glyph_extents = {
let x_min = {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
...
let ach_vend_id = (Decoder65(_input))?;
let fs_selection = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#52
fn Decoder52(_input: &mut Parser<'_>) -> Result<opentype_hhea_table, ParseError> {
let major_version = {
let inner = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#53
fn Decoder53(_input: &mut Parser<'_>, num_long_metrics: u16, num_glyphs: u16) -> Result<opentype_hmtx_table, ParseError> {
let long_metrics = {
let mut accum = Vec::new();
for _ in 0..num_long_metrics {
let next_elem = {
...
/// d#65
fn Decoder65(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
PResult::Ok(u32be(x))
}
...
/// d#80
fn Decoder80(_input: &mut Parser<'_>) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
(Decoder81(_input, point_count as u16))?
},
...
return Err(ParseError::ExcludedBranch(17869550927478639832u64));
}
};
let lo = _input.read_byte()?;
(Decoder81(_input, ((hi as u16) & 127u16) << 8u16 | (lo as u16)))?
},
...
/// d#81
fn Decoder81(_input: &mut Parser<'_>, point_count: u16) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
...
_ => {
return Err(ParseError::FailToken(15803403730818557393u64));
}
};
...
let feature_tag = (Decoder65(_input))?;
let feature = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let script_tag = (Decoder65(_input))?;
let script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let lang_sys_tag = (Decoder65(_input))?;
let lang_sys = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
_ => {
return Err(ParseError::FailToken(17349123374714965876u64));
}
})
}
...
Some((Decoder160(_input, value_format2, table_view))?)
} else {
None
};
...
/// d#160
fn Decoder160<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record1 = if value_format1.x_placement || value_format1.y_placement || value_format1.x_advance || value_format1.y_advance || value_format1.x_placement_device || value_format1.y_placement_device || value_format1.x_advance_device || value_format1.y_advance_device {
Some((Decoder163(_input, value_format1, set_view))?)
} else {
None
};
let value_record2 = if value_format2.x_placement || value_format2.y_placement || value_format2.x_advance || value_format2.y_advance || value_format2.x_placement_device || value_format2.y_placement_device || value_format2.x_advance_device || value_format2.y_advance_device {
Some((Decoder164(_input, value_format2, set_view))?)
} else {
None
};
...
/// d#163
fn Decoder163<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record = (Decoder168(_input, value_format, table_view))?;
...
let next_elem = (Decoder167(_input, value_format, table_view))?;
accum.push(next_elem)
};
accum
};
...
/// d#167
fn Decoder167<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let base_script_tag = (Decoder65(_input))?;
let base_script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let base_lang_sys_tag = (Decoder65(_input))?;
let min_max = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
_ => {
return Err(ParseError::FailToken(11046436797737227751u64));
}
};
...
/// d#196
fn Decoder196(_input: &mut Parser<'_>) -> Result<i64, ParseError> {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u64be(x)
};
...
13u16 => {
let inner = (Decoder206(_input, _platform))?;
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(6279463968646665849u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(15898809900392744567u64));
inner
}
};
let length = {
...
/// d#206
fn Decoder206(_input: &mut Parser<'_>, _platform: u16) -> Result<opentype_cmap_subtable_format12, ParseError> {
let length = {
_input.open_peek_context();
let ret = ((|| {
let _format = {
...
let r#type = (Decoder230(_input, ident.data == 2u8))?;
let machine = (Decoder231(_input, ident.data == 2u8))?;
let version = (Decoder232(_input, ident.data == 2u8))?;
...
let flags = (Decoder215(_input, ident.data == 2u8))?;
let ehsize = (Decoder233(_input, ident.data == 2u8))?;
let phentsize = (Decoder233(_input, ident.data == 2u8))?;
let phnum = (Decoder233(_input, ident.data == 2u8))?;
let shentsize = (Decoder233(_input, ident.data == 2u8))?;
let shnum = (Decoder233(_input, ident.data == 2u8))?;
let shstrndx = (Decoder233(_input, ident.data == 2u8))?;
...
/// d#211
fn Decoder211(_input: &mut Parser<'_>, is_be: bool, class: u8, phnum: u16) -> Result<Vec<elf_phdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..phnum {
...
/// d#212
fn Decoder212(_input: &mut Parser<'_>, is_be: bool, class: u8, shnum: u16) -> Result<Vec<elf_shdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..shnum {
...
/// d#213
fn Decoder213(_input: &mut Parser<'_>, r#type: u32, size: u64) -> Result<Vec<u8>, ParseError> {
PResult::Ok({
let _ = r#type;
let mut accum = Vec::new();
for _ in 0..size {
...
let name = (Decoder215(_input, is_be))?;
...
let link = (Decoder215(_input, is_be))?;
let info = (Decoder220(_input, is_be))?;
...
let inner = (Decoder215(_input, is_be))?;
let is_valid = {
let sh_type = inner;
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder221(_input, is_be))?;
...
/// d#222
fn Decoder222(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
let r#type = (Decoder215(_input, is_be))?;
let flags64 = (Decoder227(_input, is_be, class))?;
let offset = (Decoder_elf_types_elf_off(_input, is_be, class))?;
...
let flags32 = (Decoder228(_input, is_be, class))?;
...
/// d#227
fn Decoder227(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
PResult::Ok(if class == 2u8 {
Some((Decoder215(_input, is_be))?)
} else {
None
})
}
...
let class = (Decoder234(_input))?;
let data = (Decoder235(_input))?;
let version = (Decoder236(_input))?;
let os_abi = (Decoder237(_input))?;
let abi_version = (Decoder238(_input))?;
{
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#230
fn Decoder230(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
let inner = (Decoder233(_input, is_be))?;
let is_valid = {
let r#type = inner;
...
Decoder233(_input, is_be)
}

/// d#232
fn Decoder232(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
let inner = (Decoder215(_input, is_be))?;
let is_valid = {
let x = inner;
x <= 1u32
...
/// d#234
fn Decoder234(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
try_sub!(bit as u8, 48u8, 13319523888327217639u64)
};
let _o5 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 16096650375442290768u64)
};
let _o4 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 1479153625485860551u64)
};
let _o3 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 1844274570107701975u64)
};
let _o2 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 14903563845775542749u64)
};
let _o1 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 7281717462557989541u64)
};
let _o0 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 6209434968043366837u64)
};
(Decoder242(_input))?;
...
/// d#243
fn Decoder243(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#247
fn Decoder247(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16le(x)
};
...
/// d#250
fn Decoder250(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
...
return Err(ParseError::FalsifiedWhere(4316175446384649956u64));
}
};
...
return Err(ParseError::FalsifiedWhere(4598583460226006268u64));
}
};
let tag = (Decoder266(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
let ret = (Decoder267(_input))?;
_input.end_slice()?;
ret
};
let crc = {
...
return Err(ParseError::FalsifiedWhere(275550262640764009u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#257
fn Decoder257(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let literal_length_distance_alphabet_code_lengths = (Decoder263(_input, hlit, hdist, &code_length_alphabet_code_lengths))?;
...
/// d#263
fn Decoder263(_input: &mut Parser<'_>, hlit: u8, hdist: u8, code_length_alphabet_code_lengths: &[u8]) -> Result<Vec<deflate_dynamic_huffman_literal_length_distance_alphabet_code_lengths>, ParseError> {
...
0u8 => {
(Decoder265(_input, 0u8, 1u16))?
},

1u8 => {
(Decoder265(_input, 0u8, 2u16))?
},

2u8 => {
(Decoder265(_input, 0u8, 3u16))?
},

3u8 => {
(Decoder265(_input, 0u8, 4u16))?
},

4u8 => {
(Decoder265(_input, 1u8, 5u16))?
},

5u8 => {
(Decoder265(_input, 1u8, 7u16))?
},

6u8 => {
(Decoder265(_input, 2u8, 9u16))?
},

7u8 => {
(Decoder265(_input, 2u8, 13u16))?
},

8u8 => {
(Decoder265(_input, 3u8, 17u16))?
},

9u8 => {
(Decoder265(_input, 3u8, 25u16))?
},

10u8 => {
(Decoder265(_input, 4u8, 33u16))?
},

11u8 => {
(Decoder265(_input, 4u8, 49u16))?
},

12u8 => {
(Decoder265(_input, 5u8, 65u16))?
},

13u8 => {
(Decoder265(_input, 5u8, 97u16))?
},

14u8 => {
(Decoder265(_input, 6u8, 129u16))?
},

15u8 => {
(Decoder265(_input, 6u8, 193u16))?
},

16u8 => {
(Decoder265(_input, 7u8, 257u16))?
},

17u8 => {
(Decoder265(_input, 7u8, 385u16))?
},

18u8 => {
(Decoder265(_input, 8u8, 513u16))?
},

19u8 => {
(Decoder265(_input, 8u8, 769u16))?
},

20u8 => {
(Decoder265(_input, 9u8, 1025u16))?
},

21u8 => {
(Decoder265(_input, 9u8, 1537u16))?
},

22u8 => {
(Decoder265(_input, 10u8, 2049u16))?
},

23u8 => {
(Decoder265(_input, 10u8, 3073u16))?
},

24u8 => {
(Decoder265(_input, 11u8, 4097u16))?
},

25u8 => {
(Decoder265(_input, 11u8, 6145u16))?
},

26u8 => {
(Decoder265(_input, 12u8, 8193u16))?
},

27u8 => {
(Decoder265(_input, 12u8, 12289u16))?
},

28u8 => {
(Decoder265(_input, 13u8, 16385u16))?
},

29u8 => {
(Decoder265(_input, 13u8, 24577u16))?
},

30u8..=31u8 => {
return Err(ParseError::FailToken(1595897747104696027u64));
},
...
/// d#265
fn Decoder265(_input: &mut Parser<'_>, extra_bits: u8, start: u16) -> Result<deflate_distance_record0, ParseError> {
...
/// d#266
fn Decoder266(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
/// d#269
fn Decoder269(_input: &mut Parser<'_>) -> Result<Vec<gif_color_table_entry>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let compressed_profile = (Decoder297(_input))?;
...
let compressed_text = {
let zlib = (Decoder286(_input))?;
let mut buf_parser = Parser::new(slice_all(&zlib.data.inflate));
let buf_input = &mut buf_parser;
(Decoder287(buf_input))?
};
...
/// d#284
fn Decoder284(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#286
fn Decoder286(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(3379987508464424555u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#287
fn Decoder287(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
Decoder288(_input)
}
...
/// d#291
fn Decoder291(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#292
fn Decoder292(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(15547793456796641064u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#296
fn Decoder296(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#301
fn Decoder301(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
PResult::Ok((_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?))
}

/// d#302
fn Decoder_mpeg4_ftyp_data(_input: &mut Parser<'_>) -> Result<mpeg4_ftyp_data, ParseError> {
let major_brand = (Decoder301(_input))?;
let minor_version = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
if matching_ix == 0 {
let next_elem = (Decoder301(_input))?;
accum.push(next_elem)
} else {
break
}
...
let component_subtype = (Decoder301(_input))?;
...
if matching_ix == 0 {
let next_elem = (Decoder333(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#333
fn Decoder333(_input: &mut Parser<'_>) -> Result<mpeg4_dref_data_data, ParseError> {
let size_field = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
let handler_type = (Decoder301(_input))?;
let reserved = {
let arg0 = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
...
try_sub!(x, 16u64, 15786118691017431738u64)
},
...
try_sub!(x, 16u64, 16399036514137665776u64)
},
...
let item_type = (Decoder301(_input))?;
let item_name = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
if matching_ix == 0 {
let next_elem = (Decoder364(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#364
fn Decoder364(_input: &mut Parser<'_>) -> Result<jpeg_scan, ParseError> {
let segments = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#368
fn Decoder368(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
/// d#369
fn Decoder369(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
return Err(ParseError::FalsifiedWhere(10650412753233146525u64));
}
};
...
return Err(ParseError::FalsifiedWhere(15859964085544252343u64));
}
};
...
return Err(ParseError::FalsifiedWhere(653325817133119558u64));
}
};
...
return Err(ParseError::FalsifiedWhere(3349032559334020401u64));
})
}
...
/// d#382
fn Decoder382(_input: &mut Parser<'_>) -> Result<jpeg_scan_data, ParseError> {
let scan_data = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#384
fn Decoder384(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#405
fn Decoder405(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(6301432137385173939u64));
}
};
let density_x = {
let inner = {
...
return Err(ParseError::FalsifiedWhere(17762152702085771435u64));
}
};
let density_y = {
let inner = {
...
/// d#436
fn Decoder436(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#442
fn Decoder442(_input: &mut Parser<'_>) -> Result<Vec<gzip_main>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#443
fn Decoder443(_input: &mut Parser<'_>) -> Result<tar_main, ParseError> {
let contents = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#444
fn Decoder444(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#445
fn Decoder445(_input: &mut Parser<'_>) -> Result<gzip_fcomment, ParseError> {
let comment = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
let next_elem = (Decoder461(_input))?;
accum.push(next_elem)
};
Some(accum)
} else {
...
let terminator = (Decoder455(_input))?;
...
/// d#455
fn Decoder455(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(if b == 0 {
b
} else {
...
/// d#461
fn Decoder461(_input: &mut Parser<'_>) -> Result<gif_color_table_entry, ParseError> {
let r = _input.read_byte()?;
let g = _input.read_byte()?;
let b = _input.read_byte()?;
PResult::Ok(gif_color_table_entry { r, g, b })
...
/// d#467
fn Decoder467(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let tuple_var = {
let arg0 = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
/// d#469
fn Decoder469(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder470(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
if b == 0 {
b
...
/// d#470
fn Decoder470(_input: &mut Parser<'_>) -> Result<jpeg_dhp_image_component_sampling_factor, ParseError> {
let _packed_bits = _input.read_byte()?;
let horizontal = _packed_bits >> 4u8 & 15u8;
let vertical = _packed_bits & 15u8;
PResult::Ok(jpeg_dhp_image_component_sampling_factor { horizontal, vertical })
...
/// d#471
fn Decoder471(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
...
return Err(ParseError::FalsifiedWhere(17495212514432555390u64));
}
};
let num_image_components = {
let inner = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(12914822120096438372u64));
}
};
let image_components = {
let mut accum = Vec::new();
...
/// d#472
fn Decoder472(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder470(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(9709424480928378884u64));
}
};
...
/// d#474
fn Decoder474(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#475
fn Decoder475(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#478
fn Decoder478(_input: &mut Parser<'_>) -> Result<jpeg_exp_data, ParseError> {
let expand_horizontal_vertical = {
let inner = {
let _packed_bits = _input.read_byte()?;
let expand_horizontal = _packed_bits >> 4u8 & 15u8;
