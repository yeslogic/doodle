...
/// expected size: 3
/// trait-unready: multiple (2) decoders exist (d#{422, 457})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{433, 441})
#[derive(Debug, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (2) decoders exist (d#{260, 261})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 1
/// trait-unready: multiple (10) decoders exist (d#{352, 354, 365, 366, 367, 368, 369, 370, 371, 372})
#[derive(Debug, Copy, Clone)]
pub struct jpeg_soi {
marker: u8
}
...
/// expected size: 56
/// trait-unready: multiple (3) decoders exist (d#{242, 243, 244})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (16) decoders exist (d#{400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 470})
#[derive(Debug, Clone)]
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{464, 466})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (3) decoders exist (d#{393, 465, 468})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (3) decoders exist (d#{392, 467, 472})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (14) decoders exist (d#{379, 380, 381, 382, 383, 384, 385, 386, 387, 388, 389, 390, 391, 471})
#[derive(Debug, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{362, 378})
#[derive(Debug, Clone)]
...
/// expected size: 112
/// trait-unready: multiple (2) decoders exist (d#{358, 360})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{327, 329})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{201, 202})
#[derive(Debug, Clone)]
...
/// expected size: 28
/// trait-unready: multiple (2) decoders exist (d#{33, 51})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{35, 52})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (6) decoders exist (d#{156, 157, 160, 161, 164, 165})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 96
/// trait-unready: multiple (4) decoders exist (d#{251, 282, 288, 293})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (3) decoders exist (d#{237, 239, 240})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{12, 439})
#[derive(Debug, Clone)]
pub struct tar_main {
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{469, 474})
#[derive(Debug, Copy, Clone)]
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder207(_input, header.ident.data == 2u8, header.ident.class, header.phnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder208(_input, header.ident.data == 2u8, header.ident.class, header.shnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder209(_input, shdr.r#type, match shdr.size {
...
let sz = table.length as usize;
view_input.start_slice(sz)?;
let ret = (Decoder52(view_input, match vhea {
Some(ref x) => {
x
},
...
_input.skip_remainder();
PResult::Ok(opentype_table_directory_table_links { cmap, head, hhea, maxp, hmtx, name, os2, post, cvt, fpgm, loca, glyf, prep, gasp, base, gdef, gpos, gsub, fvar, gvar, hvar, kern, stat, vhea, vmtx, dsig, hdmx, vdmx })
}
...
let created = (Decoder192(_input))?;
let modified = (Decoder192(_input))?;
let glyph_extents = {
let x_min = {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
...
let ach_vend_id = (Decoder64(_input))?;
let fs_selection = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
_ => {
return Err(ParseError::FailToken(1079884235207081886u64));
}
};
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(980800817911480223u64));
inner
}
};
let minor_version = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(7801539417877429212u64));
inner
}
};
let minor_version = {
...
1u16 => {
return Err(ParseError::FailToken(14931240509007516758u64));
},
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(7343323033370781545u64));
inner
}
};
let instance_count = {
...
/// d#51
fn Decoder51(_input: &mut Parser<'_>) -> Result<opentype_hhea_table, ParseError> {
let major_version = {
let inner = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#52
fn Decoder52(_input: &mut Parser<'_>, num_long_metrics: u16, num_glyphs: u16) -> Result<opentype_hmtx_table, ParseError> {
let long_metrics = {
let mut accum = Vec::new();
for _ in 0..num_long_metrics {
let next_elem = {
...
/// d#64
fn Decoder64(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
PResult::Ok(u32be(x))
}
...
/// d#79
fn Decoder79(_input: &mut Parser<'_>) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
return Err(ParseError::ExcludedBranch(17544092807091201u64));
}
};
(0u16, [].to_vec())
},
...
return Err(ParseError::ExcludedBranch(10502127387712395480u64));
}
};
(Decoder80(_input, point_count as u16))?
},
...
(Decoder80(_input, ((hi as u16) & 127u16) << 8u16 | (lo as u16)))?
},
...
/// d#80
fn Decoder80(_input: &mut Parser<'_>, point_count: u16) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
...
7u16 => {
return Err(ParseError::FailToken(6915530142412472120u64));
},
...
let feature_tag = (Decoder64(_input))?;
let feature = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let script_tag = (Decoder64(_input))?;
let script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let lang_sys_tag = (Decoder64(_input))?;
let lang_sys = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
Some((Decoder157(_input, value_format2, table_view))?)
} else {
None
};
...
/// d#157
fn Decoder157<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record1 = if value_format1.x_placement || value_format1.y_placement || value_format1.x_advance || value_format1.y_advance || value_format1.x_placement_device || value_format1.y_placement_device || value_format1.x_advance_device || value_format1.y_advance_device {
Some((Decoder160(_input, value_format1, set_view))?)
} else {
None
};
let value_record2 = if value_format2.x_placement || value_format2.y_placement || value_format2.x_advance || value_format2.y_advance || value_format2.x_placement_device || value_format2.y_placement_device || value_format2.x_advance_device || value_format2.y_advance_device {
Some((Decoder161(_input, value_format2, set_view))?)
} else {
None
};
...
let value_record = (Decoder165(_input, value_format, table_view))?;
...
let next_elem = (Decoder164(_input, value_format, table_view))?;
accum.push(next_elem)
};
accum
};
...
let base_script_tag = (Decoder64(_input))?;
let base_script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let base_lang_sys_tag = (Decoder64(_input))?;
let min_max = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#192
fn Decoder192(_input: &mut Parser<'_>) -> Result<i64, ParseError> {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u64be(x)
};
...
13u16 => {
let inner = (Decoder202(_input, _platform))?;
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(14984809111992638634u64));
inner
}
};
let x = (_input.read_byte()?, _input.read_byte()?);
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(13404710972790825894u64));
inner
}
};
let inner = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(11323981950571132721u64));
inner
}
};
let x = (_input.read_byte()?, _input.read_byte()?);
...
/// d#202
fn Decoder202(_input: &mut Parser<'_>, _platform: u16) -> Result<opentype_cmap_subtable_format12, ParseError> {
let length = {
_input.open_peek_context();
let ret = ((|| {
let _format = {
...
let r#type = (Decoder226(_input, ident.data == 2u8))?;
let machine = (Decoder227(_input, ident.data == 2u8))?;
let version = (Decoder228(_input, ident.data == 2u8))?;
...
let flags = (Decoder211(_input, ident.data == 2u8))?;
let ehsize = (Decoder229(_input, ident.data == 2u8))?;
let phentsize = (Decoder229(_input, ident.data == 2u8))?;
let phnum = (Decoder229(_input, ident.data == 2u8))?;
let shentsize = (Decoder229(_input, ident.data == 2u8))?;
let shnum = (Decoder229(_input, ident.data == 2u8))?;
let shstrndx = (Decoder229(_input, ident.data == 2u8))?;
...
/// d#207
fn Decoder207(_input: &mut Parser<'_>, is_be: bool, class: u8, phnum: u16) -> Result<Vec<elf_phdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..phnum {
...
/// d#208
fn Decoder208(_input: &mut Parser<'_>, is_be: bool, class: u8, shnum: u16) -> Result<Vec<elf_shdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..shnum {
...
/// d#209
fn Decoder209(_input: &mut Parser<'_>, r#type: u32, size: u64) -> Result<Vec<u8>, ParseError> {
PResult::Ok({
let _ = r#type;
let mut accum = Vec::new();
for _ in 0..size {
...
let name = (Decoder211(_input, is_be))?;
let r#type = (Decoder212(_input, is_be))?;
...
let link = (Decoder211(_input, is_be))?;
let info = (Decoder216(_input, is_be))?;
...
/// d#211
fn Decoder211(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
...
/// d#212
fn Decoder212(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
let inner = (Decoder211(_input, is_be))?;
let is_valid = {
let sh_type = inner;
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder219(_input, is_be))?;
...
2u8 => {
let inner = (Decoder218(_input, is_be))?;
...
/// d#218
fn Decoder218(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#219
fn Decoder219(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#220
fn Decoder220(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#221
fn Decoder221(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
let r#type = (Decoder211(_input, is_be))?;
let flags64 = (Decoder223(_input, is_be, class))?;
let offset = (Decoder_elf_types_elf_off(_input, is_be, class))?;
...
let flags32 = (Decoder224(_input, is_be, class))?;
...
/// d#223
fn Decoder223(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
PResult::Ok(if class == 2u8 {
Some((Decoder211(_input, is_be))?)
} else {
None
})
}
...
/// d#224
fn Decoder224(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
...
let class = (Decoder230(_input))?;
let data = (Decoder231(_input))?;
let version = (Decoder232(_input))?;
let os_abi = (Decoder233(_input))?;
let abi_version = (Decoder234(_input))?;
{
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#226
fn Decoder226(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
...
/// d#227
fn Decoder227(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
Decoder229(_input, is_be)
}
...
/// d#229
fn Decoder229(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
_input.skip_align(2)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?);
...
/// d#230
fn Decoder230(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
/// d#231
fn Decoder231(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
/// d#232
fn Decoder232(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 1u8
...
/// d#233
fn Decoder233(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
_input.read_byte()
}
...
/// d#239
fn Decoder239(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#240
fn Decoder240(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#243
fn Decoder243(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16le(x)
};
...
/// d#244
fn Decoder244(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#246
fn Decoder246(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
...
return Err(ParseError::FalsifiedWhere(8812292064350598352u64));
}
};
let tag = (Decoder294(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(10645729856418057640u64));
}
};
let tag = (Decoder264(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(2908689796368760670u64));
}
};
...
/// d#253
fn Decoder253(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let literal_length_distance_alphabet_code_lengths = (Decoder259(_input, hlit, hdist, &code_length_alphabet_code_lengths))?;
...
/// d#259
fn Decoder259(_input: &mut Parser<'_>, hlit: u8, hdist: u8, code_length_alphabet_code_lengths: &[u8]) -> Result<Vec<deflate_dynamic_huffman_literal_length_distance_alphabet_code_lengths>, ParseError> {
...
0u8 => {
(Decoder261(_input, 0u8, 1u16))?
},

1u8 => {
(Decoder261(_input, 0u8, 2u16))?
},

2u8 => {
(Decoder261(_input, 0u8, 3u16))?
},

3u8 => {
(Decoder261(_input, 0u8, 4u16))?
},

4u8 => {
(Decoder261(_input, 1u8, 5u16))?
},

5u8 => {
(Decoder261(_input, 1u8, 7u16))?
},

6u8 => {
(Decoder261(_input, 2u8, 9u16))?
},

7u8 => {
(Decoder261(_input, 2u8, 13u16))?
},

8u8 => {
(Decoder261(_input, 3u8, 17u16))?
},

9u8 => {
(Decoder261(_input, 3u8, 25u16))?
},

10u8 => {
(Decoder261(_input, 4u8, 33u16))?
},

11u8 => {
(Decoder261(_input, 4u8, 49u16))?
},

12u8 => {
(Decoder261(_input, 5u8, 65u16))?
},

13u8 => {
(Decoder261(_input, 5u8, 97u16))?
},

14u8 => {
(Decoder261(_input, 6u8, 129u16))?
},

15u8 => {
(Decoder261(_input, 6u8, 193u16))?
},

16u8 => {
(Decoder261(_input, 7u8, 257u16))?
},

17u8 => {
(Decoder261(_input, 7u8, 385u16))?
},

18u8 => {
(Decoder261(_input, 8u8, 513u16))?
},

19u8 => {
(Decoder261(_input, 8u8, 769u16))?
},

20u8 => {
(Decoder261(_input, 9u8, 1025u16))?
},

21u8 => {
(Decoder261(_input, 9u8, 1537u16))?
},

22u8 => {
(Decoder261(_input, 10u8, 2049u16))?
},

23u8 => {
(Decoder261(_input, 10u8, 3073u16))?
},

24u8 => {
(Decoder261(_input, 11u8, 4097u16))?
},

25u8 => {
(Decoder261(_input, 11u8, 6145u16))?
},

26u8 => {
(Decoder261(_input, 12u8, 8193u16))?
},

27u8 => {
(Decoder261(_input, 12u8, 12289u16))?
},

28u8 => {
(Decoder261(_input, 13u8, 16385u16))?
},

29u8 => {
(Decoder261(_input, 13u8, 24577u16))?
},

30u8..=31u8 => {
return Err(ParseError::FailToken(18399269270080151498u64));
},
...
/// d#261
fn Decoder261(_input: &mut Parser<'_>, extra_bits: u8, start: u16) -> Result<deflate_distance_record0, ParseError> {
...
/// d#263
fn Decoder263(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#264
fn Decoder264(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
{
...
/// d#265
fn Decoder265(_input: &mut Parser<'_>) -> Result<Vec<gif_color_table_entry>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let compressed_profile = (Decoder293(_input))?;
...
let compressed_text = {
let zlib = (Decoder282(_input))?;
let mut buf_parser = Parser::new(slice_all(&zlib.data.inflate));
let buf_input = &mut buf_parser;
(Decoder283(buf_input))?
};
...
/// d#280
fn Decoder280(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#281
fn Decoder281(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#282
fn Decoder282(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(4290849149416714172u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#283
fn Decoder283(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
Decoder284(_input)
}

/// d#284
fn Decoder284(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
return Err(ParseError::FalsifiedWhere(14287222125204946454u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(17780439059155340308u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#294
fn Decoder294(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
/// d#297
fn Decoder297(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
PResult::Ok((_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?))
}

/// d#298
fn Decoder_mpeg4_ftyp_data(_input: &mut Parser<'_>) -> Result<mpeg4_ftyp_data, ParseError> {
let major_brand = (Decoder297(_input))?;
let minor_version = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
try_sub!(x, 16u64, 2859130192484418172u64)
},
...
try_sub!(x, 16u64, 16954835414833850385u64)
},
...
try_sub!(x, 16u64, 13705211812356460160u64)
},
...
try_sub!(x, 16u64, 1328880024623199753u64)
},
...
let component_subtype = (Decoder297(_input))?;
...
if matching_ix == 0 {
let next_elem = (Decoder329(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#329
fn Decoder329(_input: &mut Parser<'_>) -> Result<mpeg4_dref_data_data, ParseError> {
let size_field = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
let handler_type = (Decoder297(_input))?;
let reserved = {
let arg0 = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
...
let item_type = (Decoder297(_input))?;
let item_name = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
if matching_ix == 0 {
let next_elem = (Decoder360(_input))?;
accum.push(next_elem)
} else {
break
}
...
3 => {
let inner = (Decoder382(_input))?;
jpeg_frame_header::sof3(inner)
},

4 => {
let inner = (Decoder383(_input))?;
...
/// d#360
fn Decoder360(_input: &mut Parser<'_>) -> Result<jpeg_scan, ParseError> {
let segments = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#364
fn Decoder364(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
/// d#365
fn Decoder365(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#366
fn Decoder366(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#367
fn Decoder367(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#368
fn Decoder368(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#378
fn Decoder378(_input: &mut Parser<'_>) -> Result<jpeg_scan_data, ParseError> {
let scan_data = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#380
fn Decoder380(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#381
fn Decoder381(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#382
fn Decoder382(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#383
fn Decoder383(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#402
fn Decoder402(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#403
fn Decoder403(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#404
fn Decoder404(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(14238499412440345954u64));
}
};
let density_x = {
let inner = {
...
return Err(ParseError::FalsifiedWhere(14703397633096852322u64));
}
};
let density_y = {
let inner = {
...
return Err(ParseError::FalsifiedWhere(268478239438800266u64));
}
};
...
/// d#432
fn Decoder432(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#438
fn Decoder438(_input: &mut Parser<'_>) -> Result<Vec<gzip_main>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#439
fn Decoder439(_input: &mut Parser<'_>) -> Result<tar_main, ParseError> {
let contents = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#440
fn Decoder440(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#441
fn Decoder441(_input: &mut Parser<'_>) -> Result<gzip_fcomment, ParseError> {
let comment = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
let next_elem = (Decoder457(_input))?;
accum.push(next_elem)
};
Some(accum)
} else {
...
let terminator = (Decoder451(_input))?;
...
/// d#451
fn Decoder451(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(if b == 0 {
b
} else {
...
/// d#457
fn Decoder457(_input: &mut Parser<'_>) -> Result<gif_color_table_entry, ParseError> {
let r = _input.read_byte()?;
let g = _input.read_byte()?;
let b = _input.read_byte()?;
PResult::Ok(gif_color_table_entry { r, g, b })
...
/// d#463
fn Decoder463(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let tuple_var = {
let arg0 = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
/// d#465
fn Decoder465(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder466(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
if b == 0 {
b
...
/// d#466
fn Decoder466(_input: &mut Parser<'_>) -> Result<jpeg_dhp_image_component_sampling_factor, ParseError> {
let _packed_bits = _input.read_byte()?;
let horizontal = _packed_bits >> 4u8 & 15u8;
let vertical = _packed_bits & 15u8;
PResult::Ok(jpeg_dhp_image_component_sampling_factor { horizontal, vertical })
...
/// d#467
fn Decoder467(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
...
/// d#468
fn Decoder468(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder466(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
...
/// d#470
fn Decoder470(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#471
fn Decoder471(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(1371641324337791042u64));
}
};
let image_components = {
let mut accum = Vec::new();
...
/// d#474
fn Decoder474(_input: &mut Parser<'_>) -> Result<jpeg_exp_data, ParseError> {
let expand_horizontal_vertical = {
let inner = {
let _packed_bits = _input.read_byte()?;
let expand_horizontal = _packed_bits >> 4u8 & 15u8;
