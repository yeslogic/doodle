...
/// expected size: 3
/// trait-unready: multiple (2) decoders exist (d#{428, 463})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{439, 447})
#[derive(Debug, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (2) decoders exist (d#{266, 267})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 1
/// trait-unready: multiple (10) decoders exist (d#{358, 360, 371, 372, 373, 374, 375, 376, 377, 378})
#[derive(Debug, Copy, Clone)]
pub struct jpeg_soi {
marker: u8
}
...
/// expected size: 56
/// trait-unready: multiple (3) decoders exist (d#{248, 249, 250})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (16) decoders exist (d#{406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 419, 420, 476})
#[derive(Debug, Clone)]
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{470, 472})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (3) decoders exist (d#{399, 471, 474})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (3) decoders exist (d#{398, 473, 478})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (14) decoders exist (d#{385, 386, 387, 388, 389, 390, 391, 392, 393, 394, 395, 396, 397, 477})
#[derive(Debug, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{368, 384})
#[derive(Debug, Clone)]
...
/// expected size: 112
/// trait-unready: multiple (2) decoders exist (d#{364, 366})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{333, 335})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{207, 208})
#[derive(Debug, Clone)]
...
/// expected size: 28
/// trait-unready: multiple (2) decoders exist (d#{33, 53})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{35, 54})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (6) decoders exist (d#{161, 162, 165, 166, 169, 170})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 96
/// trait-unready: multiple (4) decoders exist (d#{257, 288, 294, 299})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (3) decoders exist (d#{243, 245, 246})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{12, 445})
#[derive(Debug, Clone)]
pub struct tar_main {
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{475, 480})
#[derive(Debug, Copy, Clone)]
...
let next_elem = (Decoder469(_input))?;
accum.push(next_elem)
}
};
PResult::Ok(accum)
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder213(_input, header.ident.data == 2u8, header.ident.class, header.phnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder214(_input, header.ident.data == 2u8, header.ident.class, header.shnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder215(_input, shdr.r#type, match shdr.size {
...
let sz = table.length as usize;
view_input.start_slice(sz)?;
let ret = (Decoder54(view_input, match vhea {
Some(ref x) => {
x
},
...
_input.skip_remainder();
PResult::Ok(opentype_table_directory_table_links { cmap, head, hhea, maxp, hmtx, name, os2, post, cvt, fpgm, loca, glyf, prep, gasp, base, gdef, gpos, gsub, avar, fvar, gvar, hvar, mvar, kern, stat, vhea, vmtx, dsig, hdmx, vdmx })
}
...
let created = (Decoder198(_input))?;
let modified = (Decoder198(_input))?;
let glyph_extents = {
let x_min = {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
...
let ach_vend_id = (Decoder66(_input))?;
let fs_selection = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#53
fn Decoder53(_input: &mut Parser<'_>) -> Result<opentype_hhea_table, ParseError> {
let major_version = {
let inner = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#54
fn Decoder54(_input: &mut Parser<'_>, num_long_metrics: u16, num_glyphs: u16) -> Result<opentype_hmtx_table, ParseError> {
let long_metrics = {
let mut accum = Vec::new();
for _ in 0..num_long_metrics {
let next_elem = {
...
/// d#66
fn Decoder66(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
PResult::Ok(u32be(x))
}
...
let value_tag = (Decoder66(_input))?;
...
/// d#82
fn Decoder82(_input: &mut Parser<'_>) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
(Decoder83(_input, point_count as u16))?
},
...
(Decoder83(_input, ((hi as u16) & 127u16) << 8u16 | (lo as u16)))?
},
...
/// d#83
fn Decoder83(_input: &mut Parser<'_>, point_count: u16) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(6915530142412472120u64));
inner
}
};
let minor_version = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(6949960292533894002u64));
inner
}
};
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(1347174710810305478u64));
inner
}
};
let minor_version = {
...
7u16 => {
return Err(ParseError::FailToken(6347242493551283856u64));
},
...
_ => {
return Err(ParseError::FailToken(11915580511665106140u64));
}
};
...
_ => {
return Err(ParseError::FailToken(12879845237981630531u64));
}
};
...
let feature_tag = (Decoder66(_input))?;
let feature = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let script_tag = (Decoder66(_input))?;
let script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let lang_sys_tag = (Decoder66(_input))?;
let lang_sys = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
9u16 => {
return Err(ParseError::FailToken(3249387167439447765u64));
},

_ => {
return Err(ParseError::FailToken(7744051144774795087u64));
}
})
}
...
Some((Decoder162(_input, value_format2, table_view))?)
} else {
None
};
...
/// d#162
fn Decoder162<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record1 = if value_format1.x_placement || value_format1.y_placement || value_format1.x_advance || value_format1.y_advance || value_format1.x_placement_device || value_format1.y_placement_device || value_format1.x_advance_device || value_format1.y_advance_device {
Some((Decoder165(_input, value_format1, set_view))?)
} else {
None
};
let value_record2 = if value_format2.x_placement || value_format2.y_placement || value_format2.x_advance || value_format2.y_advance || value_format2.x_placement_device || value_format2.y_placement_device || value_format2.x_advance_device || value_format2.y_advance_device {
Some((Decoder166(_input, value_format2, set_view))?)
} else {
None
};
...
/// d#166
fn Decoder166<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record = (Decoder170(_input, value_format, table_view))?;
...
let next_elem = (Decoder169(_input, value_format, table_view))?;
accum.push(next_elem)
};
accum
};
...
/// d#170
fn Decoder170<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(9342187932533045817u64));
inner
}
};
...
let base_script_tag = (Decoder66(_input))?;
let base_script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let base_lang_sys_tag = (Decoder66(_input))?;
let min_max = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
_ => {
return Err(ParseError::FailToken(2688427941405105545u64));
}
};
...
_ => {
return Err(ParseError::FailToken(6279463968646665849u64));
}
};
...
/// d#198
fn Decoder198(_input: &mut Parser<'_>) -> Result<i64, ParseError> {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u64be(x)
};
...
13u16 => {
let inner = (Decoder208(_input, _platform))?;
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(1179945139148562335u64));
inner
}
};
let x = (_input.read_byte()?, _input.read_byte()?);
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(3270685119814653163u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(6821845925776570829u64));
inner
}
};
let inner = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(6495907546257147840u64));
inner
}
};
let length = {
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(10310785543736156275u64));
inner
}
};
let length = {
...
/// d#208
fn Decoder208(_input: &mut Parser<'_>, _platform: u16) -> Result<opentype_cmap_subtable_format12, ParseError> {
let length = {
_input.open_peek_context();
let ret = ((|| {
let _format = {
...
let r#type = (Decoder232(_input, ident.data == 2u8))?;
let machine = (Decoder233(_input, ident.data == 2u8))?;
let version = (Decoder234(_input, ident.data == 2u8))?;
...
let flags = (Decoder217(_input, ident.data == 2u8))?;
let ehsize = (Decoder235(_input, ident.data == 2u8))?;
let phentsize = (Decoder235(_input, ident.data == 2u8))?;
let phnum = (Decoder235(_input, ident.data == 2u8))?;
let shentsize = (Decoder235(_input, ident.data == 2u8))?;
let shnum = (Decoder235(_input, ident.data == 2u8))?;
let shstrndx = (Decoder235(_input, ident.data == 2u8))?;
...
/// d#213
fn Decoder213(_input: &mut Parser<'_>, is_be: bool, class: u8, phnum: u16) -> Result<Vec<elf_phdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..phnum {
...
/// d#214
fn Decoder214(_input: &mut Parser<'_>, is_be: bool, class: u8, shnum: u16) -> Result<Vec<elf_shdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..shnum {
...
/// d#215
fn Decoder215(_input: &mut Parser<'_>, r#type: u32, size: u64) -> Result<Vec<u8>, ParseError> {
PResult::Ok({
let _ = r#type;
let mut accum = Vec::new();
for _ in 0..size {
...
let name = (Decoder217(_input, is_be))?;
let r#type = (Decoder218(_input, is_be))?;
...
let link = (Decoder217(_input, is_be))?;
let info = (Decoder222(_input, is_be))?;
...
/// d#217
fn Decoder217(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
...
/// d#218
fn Decoder218(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
let inner = (Decoder217(_input, is_be))?;
let is_valid = {
let sh_type = inner;
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder225(_input, is_be))?;
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder223(_input, is_be))?;
elf_types_elf_off::Off32(inner)
},

2u8 => {
let inner = (Decoder224(_input, is_be))?;
...
/// d#222
fn Decoder222(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
Decoder217(_input, is_be)
}

/// d#223
fn Decoder223(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#224
fn Decoder224(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#226
fn Decoder226(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
let r#type = (Decoder217(_input, is_be))?;
let flags64 = (Decoder229(_input, is_be, class))?;
let offset = (Decoder_elf_types_elf_off(_input, is_be, class))?;
...
let flags32 = (Decoder230(_input, is_be, class))?;
...
/// d#229
fn Decoder229(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
PResult::Ok(if class == 2u8 {
Some((Decoder217(_input, is_be))?)
} else {
None
})
}
...
/// d#230
fn Decoder230(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
...
let class = (Decoder236(_input))?;
let data = (Decoder237(_input))?;
let version = (Decoder238(_input))?;
let os_abi = (Decoder239(_input))?;
let abi_version = (Decoder240(_input))?;
{
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#232
fn Decoder232(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
let inner = (Decoder235(_input, is_be))?;
let is_valid = {
let r#type = inner;
...
return Err(ParseError::FalsifiedWhere(8862619478422395719u64));
})
}

/// d#233
fn Decoder233(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
Decoder235(_input, is_be)
}

/// d#234
fn Decoder234(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
let inner = (Decoder217(_input, is_be))?;
let is_valid = {
let x = inner;
x <= 1u32
...
/// d#236
fn Decoder236(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
return Err(ParseError::FalsifiedWhere(13264741506377240721u64));
})
}

/// d#238
fn Decoder238(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 1u8
...
try_sub!(bit as u8, 48u8, 12540117573097456360u64)
};
let _o8 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 5955168674639093440u64)
};
let _o7 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 824589811577025210u64)
};
let _o6 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 14339975513692068616u64)
};
let _o5 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 12668500753644823654u64)
};
let _o4 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 4839194687019048322u64)
};
let _o3 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 1969670610881234889u64)
};
let _o2 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 15510952803379905659u64)
};
let _o1 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 16474038368490899078u64)
};
let _o0 = {
let bit = {
let b = _input.read_byte()?;
...
try_sub!(bit as u8, 48u8, 7832192330748800109u64)
};
(Decoder244(_input))?;
...
/// d#244
fn Decoder244(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
...
/// d#245
fn Decoder245(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#246
fn Decoder246(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#249
fn Decoder249(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16le(x)
};
...
/// d#250
fn Decoder250(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#252
fn Decoder252(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
...
return Err(ParseError::FalsifiedWhere(11490274700962832028u64));
}
};
let tag = (Decoder300(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(15680765559661576738u64));
}
};
let tag = (Decoder270(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(1587806253186841834u64));
}
};
let tag = (Decoder268(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(11297314001547702431u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#259
fn Decoder259(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let literal_length_distance_alphabet_code_lengths = (Decoder265(_input, hlit, hdist, &code_length_alphabet_code_lengths))?;
...
/// d#265
fn Decoder265(_input: &mut Parser<'_>, hlit: u8, hdist: u8, code_length_alphabet_code_lengths: &[u8]) -> Result<Vec<deflate_dynamic_huffman_literal_length_distance_alphabet_code_lengths>, ParseError> {
...
0u8 => {
(Decoder267(_input, 0u8, 1u16))?
},

1u8 => {
(Decoder267(_input, 0u8, 2u16))?
},

2u8 => {
(Decoder267(_input, 0u8, 3u16))?
},

3u8 => {
(Decoder267(_input, 0u8, 4u16))?
},

4u8 => {
(Decoder267(_input, 1u8, 5u16))?
},

5u8 => {
(Decoder267(_input, 1u8, 7u16))?
},

6u8 => {
(Decoder267(_input, 2u8, 9u16))?
},

7u8 => {
(Decoder267(_input, 2u8, 13u16))?
},

8u8 => {
(Decoder267(_input, 3u8, 17u16))?
},

9u8 => {
(Decoder267(_input, 3u8, 25u16))?
},

10u8 => {
(Decoder267(_input, 4u8, 33u16))?
},

11u8 => {
(Decoder267(_input, 4u8, 49u16))?
},

12u8 => {
(Decoder267(_input, 5u8, 65u16))?
},

13u8 => {
(Decoder267(_input, 5u8, 97u16))?
},

14u8 => {
(Decoder267(_input, 6u8, 129u16))?
},

15u8 => {
(Decoder267(_input, 6u8, 193u16))?
},

16u8 => {
(Decoder267(_input, 7u8, 257u16))?
},

17u8 => {
(Decoder267(_input, 7u8, 385u16))?
},

18u8 => {
(Decoder267(_input, 8u8, 513u16))?
},

19u8 => {
(Decoder267(_input, 8u8, 769u16))?
},

20u8 => {
(Decoder267(_input, 9u8, 1025u16))?
},

21u8 => {
(Decoder267(_input, 9u8, 1537u16))?
},

22u8 => {
(Decoder267(_input, 10u8, 2049u16))?
},

23u8 => {
(Decoder267(_input, 10u8, 3073u16))?
},

24u8 => {
(Decoder267(_input, 11u8, 4097u16))?
},

25u8 => {
(Decoder267(_input, 11u8, 6145u16))?
},

26u8 => {
(Decoder267(_input, 12u8, 8193u16))?
},

27u8 => {
(Decoder267(_input, 12u8, 12289u16))?
},

28u8 => {
(Decoder267(_input, 13u8, 16385u16))?
},

29u8 => {
(Decoder267(_input, 13u8, 24577u16))?
},

30u8..=31u8 => {
return Err(ParseError::FailToken(14600508952542130472u64));
},
...
/// d#267
fn Decoder267(_input: &mut Parser<'_>, extra_bits: u8, start: u16) -> Result<deflate_distance_record0, ParseError> {
...
/// d#268
fn Decoder268(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
/// d#269
fn Decoder269(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#270
fn Decoder270(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
{
...
/// d#271
fn Decoder271(_input: &mut Parser<'_>) -> Result<Vec<gif_color_table_entry>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let compressed_profile = (Decoder299(_input))?;
...
let compressed_text = {
let zlib = (Decoder288(_input))?;
let mut buf_parser = Parser::new(slice_all(&zlib.data.inflate));
let buf_input = &mut buf_parser;
(Decoder289(buf_input))?
};
...
/// d#287
fn Decoder287(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#288
fn Decoder288(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(15508559755986719975u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#289
fn Decoder289(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
Decoder290(_input)
}

/// d#290
fn Decoder290(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#294
fn Decoder294(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(6886530205512451724u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#295
fn Decoder295(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#297
fn Decoder297(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#298
fn Decoder298(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#299
fn Decoder299(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
/// d#300
fn Decoder300(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let major_brand = (Decoder303(_input))?;
let minor_version = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
if matching_ix == 0 {
let next_elem = (Decoder303(_input))?;
accum.push(next_elem)
} else {
break
}
...
let component_subtype = (Decoder303(_input))?;
...
/// d#335
fn Decoder335(_input: &mut Parser<'_>) -> Result<mpeg4_dref_data_data, ParseError> {
let size_field = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
let handler_type = (Decoder303(_input))?;
let reserved = {
let arg0 = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
...
try_sub!(x, 16u64, 17863486658382945784u64)
},
...
try_sub!(x, 16u64, 8584109755265226714u64)
},
...
let item_type = (Decoder303(_input))?;
let item_name = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
if matching_ix == 0 {
let next_elem = (Decoder366(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#360
fn Decoder360(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#366
fn Decoder366(_input: &mut Parser<'_>) -> Result<jpeg_scan, ParseError> {
let segments = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#370
fn Decoder370(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
/// d#371
fn Decoder371(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#372
fn Decoder372(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#373
fn Decoder373(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
return Err(ParseError::FalsifiedWhere(15412400192383838763u64));
}
};
...
/// d#384
fn Decoder384(_input: &mut Parser<'_>) -> Result<jpeg_scan_data, ParseError> {
let scan_data = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#386
fn Decoder386(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#387
fn Decoder387(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#388
fn Decoder388(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(12845528861092334564u64));
}
};
let num_lines = {
let x = (_input.read_byte()?, _input.read_byte()?);
...
return Err(ParseError::FalsifiedWhere(8497774971318424699u64));
}
};
let num_image_components = {
let inner = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(13677998342346693652u64));
}
};
let image_components = {
let mut accum = Vec::new();
...
return Err(ParseError::FalsifiedWhere(18442161777584514946u64));
}
};
...
/// d#407
fn Decoder407(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#408
fn Decoder408(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#409
fn Decoder409(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(18210277358428599455u64));
}
};
...
/// d#438
fn Decoder438(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#444
fn Decoder444(_input: &mut Parser<'_>) -> Result<Vec<gzip_main>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let fname = if header.file_flags.fname {
Some((Decoder446(_input))?)
} else {
None
};
let fcomment = if header.file_flags.fcomment {
Some((Decoder447(_input))?)
} else {
None
};
let fhcrc = if header.file_flags.fhcrc {
...
/// d#445
fn Decoder445(_input: &mut Parser<'_>) -> Result<tar_main, ParseError> {
let contents = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#446
fn Decoder446(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#447
fn Decoder447(_input: &mut Parser<'_>) -> Result<gzip_fcomment, ParseError> {
let comment = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
let next_elem = (Decoder463(_input))?;
accum.push(next_elem)
};
Some(accum)
} else {
...
let terminator = (Decoder457(_input))?;
...
/// d#457
fn Decoder457(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(if b == 0 {
b
} else {
...
/// d#463
fn Decoder463(_input: &mut Parser<'_>) -> Result<gif_color_table_entry, ParseError> {
let r = _input.read_byte()?;
let g = _input.read_byte()?;
let b = _input.read_byte()?;
PResult::Ok(gif_color_table_entry { r, g, b })
...
/// d#469
fn Decoder469(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let tuple_var = {
let arg0 = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
/// d#471
fn Decoder471(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder472(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
if b == 0 {
b
...
/// d#472
fn Decoder472(_input: &mut Parser<'_>) -> Result<jpeg_dhp_image_component_sampling_factor, ParseError> {
let _packed_bits = _input.read_byte()?;
let horizontal = _packed_bits >> 4u8 & 15u8;
let vertical = _packed_bits & 15u8;
PResult::Ok(jpeg_dhp_image_component_sampling_factor { horizontal, vertical })
...
/// d#473
fn Decoder473(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
...
return Err(ParseError::FalsifiedWhere(13844417374108844899u64));
}
};
let num_image_components = {
let inner = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(6192442272039337687u64));
}
};
let image_components = {
let mut accum = Vec::new();
for _ in 0..num_image_components {
let next_elem = (Decoder474(_input))?;
accum.push(next_elem)
};
accum
};
...
return Err(ParseError::FalsifiedWhere(13840694103855630215u64));
}
};
...
/// d#476
fn Decoder476(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#477
fn Decoder477(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#478
fn Decoder478(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
let sample_precision = _input.read_byte()?;
let num_lines = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
return Err(ParseError::FalsifiedWhere(7324022130208344343u64));
}
};
let num_image_components = {
let inner = _input.read_byte()?;
...
return Err(ParseError::FalsifiedWhere(3289883751054141021u64));
}
};
let image_components = {
let mut accum = Vec::new();
...
/// d#480
fn Decoder480(_input: &mut Parser<'_>) -> Result<jpeg_exp_data, ParseError> {
let expand_horizontal_vertical = {
let inner = {
let _packed_bits = _input.read_byte()?;
let expand_horizontal = _packed_bits >> 4u8 & 15u8;
