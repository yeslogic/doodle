...
/// expected size: 3
/// trait-unready: multiple (2) decoders exist (d#{419, 454})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{430, 438})
#[derive(Debug, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (2) decoders exist (d#{257, 258})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 1
/// trait-unready: multiple (10) decoders exist (d#{349, 351, 362, 363, 364, 365, 366, 367, 368, 369})
#[derive(Debug, Copy, Clone)]
pub struct jpeg_soi {
marker: u8
}
...
/// expected size: 56
/// trait-unready: multiple (3) decoders exist (d#{239, 240, 241})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (16) decoders exist (d#{397, 398, 399, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 467})
#[derive(Debug, Clone)]
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{461, 463})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 4
/// trait-unready: multiple (3) decoders exist (d#{390, 462, 465})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (3) decoders exist (d#{389, 464, 469})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (14) decoders exist (d#{376, 377, 378, 379, 380, 381, 382, 383, 384, 385, 386, 387, 388, 468})
#[derive(Debug, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{359, 375})
#[derive(Debug, Clone)]
...
/// expected size: 112
/// trait-unready: multiple (2) decoders exist (d#{355, 357})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{324, 326})
#[derive(Debug, Clone)]
...
/// expected size: 40
/// trait-unready: multiple (2) decoders exist (d#{198, 199})
#[derive(Debug, Clone)]
...
/// expected size: 28
/// trait-unready: multiple (2) decoders exist (d#{33, 50})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 48
/// trait-unready: multiple (2) decoders exist (d#{35, 51})
#[derive(Debug, Clone)]
...
/// expected size: 32
/// trait-unready: multiple (6) decoders exist (d#{150, 151, 154, 155, 158, 159})
#[derive(Debug, Copy, Clone)]
...
/// expected size: 96
/// trait-unready: multiple (4) decoders exist (d#{248, 279, 285, 290})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (3) decoders exist (d#{234, 236, 237})
#[derive(Debug, Clone)]
...
/// expected size: 24
/// trait-unready: multiple (2) decoders exist (d#{12, 436})
#[derive(Debug, Clone)]
pub struct tar_main {
...
/// expected size: 2
/// trait-unready: multiple (2) decoders exist (d#{466, 471})
#[derive(Debug, Copy, Clone)]
...
let next_elem = (Decoder460(_input))?;
accum.push(next_elem)
}
};
PResult::Ok(accum)
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder204(_input, header.ident.data == 2u8, header.ident.class, header.phnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder205(_input, header.ident.data == 2u8, header.ident.class, header.shnum))?;
_input.close_peek_context()?;
Some(ret)
} else {
None
...
let _is_advance = _input.advance_or_seek(tgt_offset)?;
let ret = (Decoder206(_input, shdr.r#type, match shdr.size {
...
let sz = table.length as usize;
view_input.start_slice(sz)?;
let ret = (Decoder51(view_input, match vhea {
Some(ref x) => {
x
},
...
_input.skip_remainder();
PResult::Ok(opentype_table_directory_table_links { cmap, head, hhea, maxp, hmtx, name, os2, post, cvt, fpgm, loca, glyf, prep, gasp, base, gdef, gpos, gsub, fvar, gvar, kern, stat, vhea, vmtx, dsig, hdmx, vdmx })
}
...
let created = (Decoder189(_input))?;
let modified = (Decoder189(_input))?;
let glyph_extents = {
let x_min = {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
...
let ach_vend_id = (Decoder63(_input))?;
let fs_selection = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#50
fn Decoder50(_input: &mut Parser<'_>) -> Result<opentype_hhea_table, ParseError> {
let major_version = {
let inner = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#51
fn Decoder51(_input: &mut Parser<'_>, num_long_metrics: u16, num_glyphs: u16) -> Result<opentype_hmtx_table, ParseError> {
let long_metrics = {
let mut accum = Vec::new();
for _ in 0..num_long_metrics {
let next_elem = {
...
/// d#63
fn Decoder63(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
PResult::Ok(u32be(x))
}
...
/// d#73
fn Decoder73(_input: &mut Parser<'_>) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
_ => {
return Err(ParseError::ExcludedBranch(14959848987246965519u64));
}
};
_input.close_peek_context()?;
ret
...
return Err(ParseError::ExcludedBranch(12275201028130973875u64));
}
};
(0u16, [].to_vec())
},
...
return Err(ParseError::ExcludedBranch(16097120758067046920u64));
}
};
(Decoder74(_input, point_count as u16))?
},
...
return Err(ParseError::ExcludedBranch(9331632426086095927u64));
}
};
let lo = _input.read_byte()?;
(Decoder74(_input, ((hi as u16) & 127u16) << 8u16 | (lo as u16)))?
},

_ => {
return Err(ParseError::ExcludedBranch(9092905213558799443u64));
}
})
}

/// d#74
fn Decoder74(_input: &mut Parser<'_>, point_count: u16) -> Result<(u16, Vec<opentype_var_packed_point_numbers_runs>), ParseError> {
...
let feature_tag = (Decoder63(_input))?;
let feature = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let script_tag = (Decoder63(_input))?;
let script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let lang_sys_tag = (Decoder63(_input))?;
let lang_sys = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
Some((Decoder151(_input, value_format2, table_view))?)
} else {
None
};
...
/// d#151
fn Decoder151<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record1 = if value_format1.x_placement || value_format1.y_placement || value_format1.x_advance || value_format1.y_advance || value_format1.x_placement_device || value_format1.y_placement_device || value_format1.x_advance_device || value_format1.y_advance_device {
Some((Decoder154(_input, value_format1, set_view))?)
} else {
None
};
let value_record2 = if value_format2.x_placement || value_format2.y_placement || value_format2.x_advance || value_format2.y_advance || value_format2.x_placement_device || value_format2.y_placement_device || value_format2.x_advance_device || value_format2.y_advance_device {
Some((Decoder155(_input, value_format2, set_view))?)
} else {
None
};
...
/// d#154
fn Decoder154<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#155
fn Decoder155<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let value_record = (Decoder159(_input, value_format, table_view))?;
...
let next_elem = (Decoder158(_input, value_format, table_view))?;
accum.push(next_elem)
};
accum
};
...
/// d#158
fn Decoder158<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#159
fn Decoder159<'input>(_input: &mut Parser<'input>, flags: opentype_layout_value_format_flags, table_view: View<'input>) -> Result<opentype_layout_value_record, ParseError> {
let x_placement = if flags.x_placement {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(3249387167439447765u64));
inner
}
};
...
let base_script_tag = (Decoder63(_input))?;
let base_script = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
let base_lang_sys_tag = (Decoder63(_input))?;
let min_max = {
let offset = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#189
fn Decoder189(_input: &mut Parser<'_>) -> Result<i64, ParseError> {
let raw = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u64be(x)
};
...
13u16 => {
let inner = (Decoder199(_input, _platform))?;
...
log::error!("expect-level value assertion failed: {}", ParseError::FalsifiedWhere(11046436797737227751u64));
inner
}
};
let length = {
...
/// d#199
fn Decoder199(_input: &mut Parser<'_>, _platform: u16) -> Result<opentype_cmap_subtable_format12, ParseError> {
let length = {
_input.open_peek_context();
let ret = ((|| {
{
...
let r#type = (Decoder223(_input, ident.data == 2u8))?;
let machine = (Decoder224(_input, ident.data == 2u8))?;
let version = (Decoder225(_input, ident.data == 2u8))?;
...
let flags = (Decoder208(_input, ident.data == 2u8))?;
let ehsize = (Decoder226(_input, ident.data == 2u8))?;
let phentsize = (Decoder226(_input, ident.data == 2u8))?;
let phnum = (Decoder226(_input, ident.data == 2u8))?;
let shentsize = (Decoder226(_input, ident.data == 2u8))?;
let shnum = (Decoder226(_input, ident.data == 2u8))?;
let shstrndx = (Decoder226(_input, ident.data == 2u8))?;
...
/// d#204
fn Decoder204(_input: &mut Parser<'_>, is_be: bool, class: u8, phnum: u16) -> Result<Vec<elf_phdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..phnum {
...
/// d#205
fn Decoder205(_input: &mut Parser<'_>, is_be: bool, class: u8, shnum: u16) -> Result<Vec<elf_shdr>, ParseError> {
let mut accum = Vec::new();
for _ in 0..shnum {
...
/// d#206
fn Decoder206(_input: &mut Parser<'_>, r#type: u32, size: u64) -> Result<Vec<u8>, ParseError> {
PResult::Ok({
let _ = r#type;
let mut accum = Vec::new();
for _ in 0..size {
...
let name = (Decoder208(_input, is_be))?;
let r#type = (Decoder209(_input, is_be))?;
...
let link = (Decoder208(_input, is_be))?;
let info = (Decoder213(_input, is_be))?;
...
/// d#208
fn Decoder208(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
...
/// d#209
fn Decoder209(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
...
PResult::Ok(match class {
1u8 => {
let inner = (Decoder214(_input, is_be))?;
...
/// d#213
fn Decoder213(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
Decoder208(_input, is_be)
}

/// d#214
fn Decoder214(_input: &mut Parser<'_>, is_be: bool) -> Result<u32, ParseError> {
_input.skip_align(4)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#215
fn Decoder215(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
/// d#217
fn Decoder217(_input: &mut Parser<'_>, is_be: bool) -> Result<u64, ParseError> {
_input.skip_align(8)?;
PResult::Ok(match is_be {
true => {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
...
let r#type = (Decoder208(_input, is_be))?;
let flags64 = (Decoder220(_input, is_be, class))?;
let offset = (Decoder_elf_types_elf_off(_input, is_be, class))?;
...
let flags32 = (Decoder221(_input, is_be, class))?;
...
/// d#220
fn Decoder220(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
PResult::Ok(if class == 2u8 {
Some((Decoder208(_input, is_be))?)
} else {
None
})
}
...
/// d#221
fn Decoder221(_input: &mut Parser<'_>, is_be: bool, class: u8) -> Result<Option<u32>, ParseError> {
...
let class = (Decoder227(_input))?;
let data = (Decoder228(_input))?;
let version = (Decoder229(_input))?;
let os_abi = (Decoder230(_input))?;
let abi_version = (Decoder231(_input))?;
{
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#223
fn Decoder223(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
...
/// d#224
fn Decoder224(_input: &mut Parser<'_>, is_be: bool) -> Result<u16, ParseError> {
Decoder226(_input, is_be)
}
...
/// d#228
fn Decoder228(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 2u8
...
/// d#229
fn Decoder229(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let inner = _input.read_byte()?;
let is_valid = {
let x = inner;
x <= 1u8
...
/// d#236
fn Decoder236(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#237
fn Decoder237(_input: &mut Parser<'_>) -> Result<tar_ascii_string_opt0_nonempty, ParseError> {
let string = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#240
fn Decoder240(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16le(x)
};
...
/// d#241
fn Decoder241(_input: &mut Parser<'_>) -> Result<tiff_ifd_le, ParseError> {
let num_fields = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
};
...
/// d#243
fn Decoder243(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
...
return Err(ParseError::FalsifiedWhere(3950014938140253048u64));
}
};
let tag = (Decoder291(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
return Err(ParseError::FalsifiedWhere(10046433636842398056u64));
}
};
let tag = (Decoder261(_input))?;
let data = {
let sz = length as usize;
_input.start_slice(sz)?;
...
let literal_length_distance_alphabet_code_lengths = (Decoder256(_input, hlit, hdist, &code_length_alphabet_code_lengths))?;
...
/// d#256
fn Decoder256(_input: &mut Parser<'_>, hlit: u8, hdist: u8, code_length_alphabet_code_lengths: &[u8]) -> Result<Vec<deflate_dynamic_huffman_literal_length_distance_alphabet_code_lengths>, ParseError> {
...
0u8 => {
(Decoder258(_input, 0u8, 1u16))?
},

1u8 => {
(Decoder258(_input, 0u8, 2u16))?
},

2u8 => {
(Decoder258(_input, 0u8, 3u16))?
},

3u8 => {
(Decoder258(_input, 0u8, 4u16))?
},

4u8 => {
(Decoder258(_input, 1u8, 5u16))?
},

5u8 => {
(Decoder258(_input, 1u8, 7u16))?
},

6u8 => {
(Decoder258(_input, 2u8, 9u16))?
},

7u8 => {
(Decoder258(_input, 2u8, 13u16))?
},

8u8 => {
(Decoder258(_input, 3u8, 17u16))?
},

9u8 => {
(Decoder258(_input, 3u8, 25u16))?
},

10u8 => {
(Decoder258(_input, 4u8, 33u16))?
},

11u8 => {
(Decoder258(_input, 4u8, 49u16))?
},

12u8 => {
(Decoder258(_input, 5u8, 65u16))?
},

13u8 => {
(Decoder258(_input, 5u8, 97u16))?
},

14u8 => {
(Decoder258(_input, 6u8, 129u16))?
},

15u8 => {
(Decoder258(_input, 6u8, 193u16))?
},

16u8 => {
(Decoder258(_input, 7u8, 257u16))?
},

17u8 => {
(Decoder258(_input, 7u8, 385u16))?
},

18u8 => {
(Decoder258(_input, 8u8, 513u16))?
},

19u8 => {
(Decoder258(_input, 8u8, 769u16))?
},

20u8 => {
(Decoder258(_input, 9u8, 1025u16))?
},

21u8 => {
(Decoder258(_input, 9u8, 1537u16))?
},

22u8 => {
(Decoder258(_input, 10u8, 2049u16))?
},

23u8 => {
(Decoder258(_input, 10u8, 3073u16))?
},

24u8 => {
(Decoder258(_input, 11u8, 4097u16))?
},

25u8 => {
(Decoder258(_input, 11u8, 6145u16))?
},

26u8 => {
(Decoder258(_input, 12u8, 8193u16))?
},

27u8 => {
(Decoder258(_input, 12u8, 12289u16))?
},

28u8 => {
(Decoder258(_input, 13u8, 16385u16))?
},

29u8 => {
(Decoder258(_input, 13u8, 24577u16))?
},

30u8..=31u8 => {
return Err(ParseError::FailToken(275550262640764009u64));
},
...
/// d#258
fn Decoder258(_input: &mut Parser<'_>, extra_bits: u8, start: u16) -> Result<deflate_distance_record0, ParseError> {
...
/// d#260
fn Decoder260(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#261
fn Decoder261(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
{
...
/// d#262
fn Decoder262(_input: &mut Parser<'_>) -> Result<Vec<gif_color_table_entry>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
let compressed_profile = (Decoder290(_input))?;
...
return Err(ParseError::FalsifiedWhere(11225936372640404826u64));
}
};
...
let compressed_text = {
let zlib = (Decoder279(_input))?;
let mut buf_parser = Parser::new(slice_all(&zlib.data.inflate));
let buf_input = &mut buf_parser;
(Decoder280(buf_input))?
};
...
/// d#277
fn Decoder277(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#278
fn Decoder278(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#279
fn Decoder279(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(10367781439645737926u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#280
fn Decoder280(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
Decoder281(_input)
}

/// d#281
fn Decoder281(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#282
fn Decoder282(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#283
fn Decoder283(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let reps_left = {
_input.open_peek_context();
...
/// d#285
fn Decoder285(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(16731067715008063623u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#286
fn Decoder286(_input: &mut Parser<'_>) -> Result<Vec<char>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#290
fn Decoder290(_input: &mut Parser<'_>) -> Result<zlib_main, ParseError> {
let compression_method_flags = {
let inner = {
let _packed_bits = _input.read_byte()?;
let compression_info = _packed_bits >> 4u8 & 15u8;
...
return Err(ParseError::FalsifiedWhere(7258731811542513498u64));
}
};
let flags = {
let _packed_bits = _input.read_byte()?;
...
/// d#291
fn Decoder291(_input: &mut Parser<'_>) -> Result<(u8, u8, u8, u8), ParseError> {
let arg0 = {
let b = _input.read_byte()?;
if b == 73 {
b
...
let major_brand = (Decoder294(_input))?;
let minor_version = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
if matching_ix == 0 {
let next_elem = (Decoder294(_input))?;
accum.push(next_elem)
} else {
break
}
...
try_sub!(x, 16u64, 10248009767256971850u64)
},
...
let component_subtype = (Decoder294(_input))?;
...
if matching_ix == 0 {
let next_elem = (Decoder326(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#326
fn Decoder326(_input: &mut Parser<'_>) -> Result<mpeg4_dref_data_data, ParseError> {
let size_field = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
};
...
let handler_type = (Decoder294(_input))?;
let reserved = {
let arg0 = {
let x = (_input.read_byte()?, _input.read_byte()?, _input.read_byte()?, _input.read_byte()?);
u32be(x)
...
let item_type = (Decoder294(_input))?;
let item_name = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
if matching_ix == 0 {
let next_elem = (Decoder357(_input))?;
accum.push(next_elem)
} else {
break
}
...
/// d#351
fn Decoder351(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#357
fn Decoder357(_input: &mut Parser<'_>) -> Result<jpeg_scan, ParseError> {
let segments = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#361
fn Decoder361(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let tree_index = {
_input.open_peek_context();
{
let ret = match _input.read_byte()? {
...
/// d#362
fn Decoder362(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#364
fn Decoder364(_input: &mut Parser<'_>) -> Result<jpeg_soi, ParseError> {
{
let b = _input.read_byte()?;
if b == 255 {
b
...
/// d#375
fn Decoder375(_input: &mut Parser<'_>) -> Result<jpeg_scan_data, ParseError> {
let scan_data = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#379
fn Decoder379(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
return Err(ParseError::FalsifiedWhere(16948595091237008543u64));
}
};
let density_x = {
let inner = {
...
return Err(ParseError::FalsifiedWhere(108256050843388088u64));
}
};
let density_y = {
let inner = {
...
return Err(ParseError::FalsifiedWhere(7834223795690054720u64));
}
};
...
/// d#429
fn Decoder429(_input: &mut Parser<'_>) -> Result<Vec<u8>, ParseError> {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#435
fn Decoder435(_input: &mut Parser<'_>) -> Result<Vec<gzip_main>, ParseError> {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
_input.open_peek_context();
...
/// d#436
fn Decoder436(_input: &mut Parser<'_>) -> Result<tar_main, ParseError> {
let contents = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
let matching_ix = {
...
/// d#438
fn Decoder438(_input: &mut Parser<'_>) -> Result<gzip_fcomment, ParseError> {
let comment = {
let chars = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
let next_elem = (Decoder454(_input))?;
accum.push(next_elem)
};
Some(accum)
} else {
...
let terminator = (Decoder448(_input))?;
...
/// d#448
fn Decoder448(_input: &mut Parser<'_>) -> Result<u8, ParseError> {
let b = _input.read_byte()?;
PResult::Ok(if b == 0 {
b
} else {
...
/// d#454
fn Decoder454(_input: &mut Parser<'_>) -> Result<gif_color_table_entry, ParseError> {
let r = _input.read_byte()?;
let g = _input.read_byte()?;
let b = _input.read_byte()?;
PResult::Ok(gif_color_table_entry { r, g, b })
...
/// d#460
fn Decoder460(_input: &mut Parser<'_>) -> Result<u32, ParseError> {
let tuple_var = {
let arg0 = {
let mut accum = Vec::new();
while _input.remaining() > 0 {
...
/// d#462
fn Decoder462(_input: &mut Parser<'_>) -> Result<jpeg_sof_image_component, ParseError> {
let id = _input.read_byte()?;
let sampling_factor = (Decoder463(_input))?;
let quantization_table_id = {
let b = _input.read_byte()?;
if b == 0 {
b
...
/// d#463
fn Decoder463(_input: &mut Parser<'_>) -> Result<jpeg_dhp_image_component_sampling_factor, ParseError> {
let _packed_bits = _input.read_byte()?;
let horizontal = _packed_bits >> 4u8 & 15u8;
let vertical = _packed_bits & 15u8;
PResult::Ok(jpeg_dhp_image_component_sampling_factor { horizontal, vertical })
...
/// d#464
fn Decoder464(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
...
let mut accum = Vec::new();
for _ in 0..num_image_components {
let next_elem = (Decoder465(_input))?;
accum.push(next_elem)
};
accum
};
...
/// d#467
fn Decoder467(_input: &mut Parser<'_>) -> Result<jpeg_app2, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#468
fn Decoder468(_input: &mut Parser<'_>) -> Result<jpeg_sof0, ParseError> {
let marker = {
{
let b = _input.read_byte()?;
if b == 255 {
...
/// d#469
fn Decoder469(_input: &mut Parser<'_>) -> Result<jpeg_sof_data, ParseError> {
let sample_precision = _input.read_byte()?;
let num_lines = {
let x = (_input.read_byte()?, _input.read_byte()?);
u16be(x)
...
/// d#471
fn Decoder471(_input: &mut Parser<'_>) -> Result<jpeg_exp_data, ParseError> {
let expand_horizontal_vertical = {
let inner = {
let _packed_bits = _input.read_byte()?;
let expand_horizontal = _packed_bits >> 4u8 & 15u8;
