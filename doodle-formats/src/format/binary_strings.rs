...
pub fn main(module: &mut FormatModule, base: &BaseModule) -> FormatRef {
    ...
    //let ascii_string = tuple([repeat1(ascii_char_strict()), is_byte(0x00)]);
    module.define_format(
        ...
        repeat1(
            Format::Union(vec![
                ...
                ("ascii".to_string(), repeat1(ascii_char_strict())),
                //("ascii-non-string".to_string(), tuple([Format::PeekNot(Box::new(ascii_string)), repeat1(ascii_char_strict())]))
            ])
            )
    )
}
