...
impl Expr {
    ...
    /// Determines whether a given expression contains an unbound reference to an external variable with identifier `name`.
    ///
    /// Returns `true` if there is such an unbound reference, and `false` otherwise (whether all references to the variable
    /// are locally-bound within `self`, or whether the variable identifier in question does not appear).
    ///
    /// # Notes
    ///
    /// This method can be used to avoid hidden shadowing between a top-level and mid-level outer context, especially
    /// in any helper function that takes in a raw `Expr` and, in its body, potentially shadows a variable external to
    /// its own AST.
    ///
    /// For example, consider the following construction:
    ///
    /// ```ignore
    /// let helper = |scope_c: Expr| -> Format {
    ///     let scope_b = record([
    ///         ("byte", ANY_BYTE),
    ///         ("data", if_then_else(expr_eq(scope_c, var("byte")), Format::EMPTY, Format::Fail)),
    ///     ]);
    ///     scope_b
    /// };
    /// /* ... */
    ...
    /// ```
    ///
    /// In this case, we represent three scopes, A > B > C. The call to `helper`
    /// within `scope_a` contains a bare variable-reference to `var("byte")`, which
    /// locally is intended to refer to the A-scoped binding of `"byte"`; however,
    /// the definition of `scope_b` causes a B-scoped binding of `"byte"` to shadow
    /// the intended evaluation of `scope_c`, which can either cause silent errors (if the types align)
    /// or cause more glaring errors (if the types do not align), which are nevertheless difficult to diagnose.
    ///
    /// The solution to this is to use `contains_unbound_reference` to determine, within
    /// `helper`, whether any immediate bindings generated by `scope_b` will lead to silent shadowing
    /// between scopes A and C, which can then be detected and corrected.
    ///
    /// # Examples
    ///
    /// ```
    /// # use doodle::Expr;
    /// # use doodle::helper::{var};
    /// assert!(var("x").contains_unbound_reference("x"));
    /// assert!(!var("x").contains_unbound_reference("y"));
    /// assert!(!Expr::Record(vec![(Label::Borrowed("x"), Expr::U16(0)), (Label::Borrowed("y"), var("x"))]).contains_unbound_reference("x"));
    /// assert!(!Expr::Lambda(Label::Borrowed("x"), Box::new(var("x")).contains_unbound_reference("x"));
    /// ```
    pub fn contains_unbound_reference(&self, name: &str) -> bool {
        match self {
            Expr::Var(vname) => *vname == name,
            Expr::Lambda(head_var, body) => {
                if head_var == name {
                    // if the head-variable is itself a shadowing binding of `name`,
                    // the lambda body cannot be influenced by externally-scoped
                    // rebindings of `name`
                    false
                } else {
                    body.contains_unbound_reference(name)
                }
            }
            Expr::Arith(_, x, y) | Expr::IntRel(_, x, y) => {
                x.contains_unbound_reference(name) || y.contains_unbound_reference(name)
            }
            Expr::Unary(_, x) => x.contains_unbound_reference(name),
            Expr::Dup(x, y) => x.contains_unbound_reference(name) || y.contains_unbound_reference(name),
            Expr::Bool(_) | Expr::U8(_) | Expr::U16(_) | Expr::U32(_) | Expr::U64(_) => false,
            Expr::Tuple(ts) => ts.iter().any(|x| x.contains_unbound_reference(name)),
            Expr::TupleProj(tup, _) => tup.contains_unbound_reference(name),
            Expr::Record(fs) => {
                for (fname, fld) in fs.iter() {
                    if fld.contains_unbound_reference(name) {
            ...
            Expr::RecordProj(rec, _) => rec.contains_unbound_reference(name),
            Expr::Variant(_, inner) => inner.contains_unbound_reference(name),
            Expr::Seq(elts) => elts.iter().any(|x| x.contains_unbound_reference(name)),
            Expr::Match(head, arms) => {
                head.contains_unbound_reference(name)
                    || arms
                        .iter()
                        .any(|(pat, x)| !pat.shadows(name) && x.contains_unbound_reference(name))
            }
            Expr::AsU8(x)
            | Expr::AsU16(x)
            | Expr::AsU32(x)
            ...
            | Expr::U64Be(x)
            | Expr::U64Le(x)
            | Expr::SeqLength(x) => x.contains_unbound_reference(name),
            Expr::EnumFromTo(s, e) => s.contains_unbound_reference(name) || e.contains_unbound_reference(name),
            Expr::SubSeq(x, s, l) | Expr::SubSeqInflate(x, s, l) => {
                x.contains_unbound_reference(name) || s.contains_unbound_reference(name) || l.contains_unbound_reference(name)
            }
            Expr::SeqIx(x, i) => x.contains_unbound_reference(name) || i.contains_unbound_reference(name),
            Expr::FlatMap(f, x) | Expr::FlatMapList(f, _, x) => {
                f.contains_unbound_reference(name) || x.contains_unbound_reference(name)
            }
            Expr::FlatMapAccum(f, z, _, x) | Expr::LeftFold(f, z, _, x) => {
                f.contains_unbound_reference(name) || z.contains_unbound_reference(name) || x.contains_unbound_reference(name)
            }
            Expr::LiftOption(opt_x) => opt_x.as_ref().is_some_and(|x| x.contains_unbound_reference(name)),
        }
    }
