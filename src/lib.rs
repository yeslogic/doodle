...
pub enum Format {
    ...
    // FIXME[epic=with-error] - introduce a distinction between error-level and warning-level validations, perhaps using util::with_err.
...
impl Format {
    ...
    pub(crate) fn to_record_format(&self) -> record_fmt::RecordFormat<'_> {
        record_fmt::RecordFormat::try_from(self).unwrap()
    }
