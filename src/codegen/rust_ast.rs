...
impl ToFragment for RustParams {
    fn to_fragment(&self) -> Fragment {
        ...
        Fragment::seq(all.map(|lab| Fragment::String(lab.clone())), Some(Fragment::string(", ")))
...
impl ToFragment for FnSig {
    fn to_fragment(&self) -> Fragment {
        ToFragment::paren_list(self.args.iter())
        .intervene(Fragment::string(" -> "), Fragment::opt(self.ret.as_ref(), RustType::to_fragment))
    }
}

pub struct RustFn { name: Label, params: Option<RustParams>, sig: FnSig, body: Vec<RustStmt> }
...
impl ToFragment for RustFn {
    fn to_fragment(&self) -> Fragment {
        ...
        Fragment::string("fn ").cat(f_name).cat(f_params).cat(f_sig).cat(Fragment::Char(' ')).cat(body)
    }
}
    ...
    pub fn to_fragment(&self) -> Fragment {
        match self {
            RustTypeDef::Enum(vars) => {
                let iter = vars.iter().map(|(vname, vdef)| Fragment::String(vname.clone()).cat(vdef.to_fragment()));
...
impl ToFragment for RustType {
    fn to_fragment(&self) -> Fragment {
        match self {
            ...
            RustType::ImplTrait(ident) => Fragment::cat(Fragment::string("impl "), Fragment::String(ident.clone())),
            RustType::AnonTuple(params) => {
                let inner = params.iter().map(|elt| elt.to_fragment());
                Fragment::seq(inner, Some(Fragment::string(", "))).delimit(Fragment::Char('('), Fragment::Char(')'))
            }
...
impl ToFragment for RustStruct {
    fn to_fragment(&self) -> Fragment {
        match self {
            ...
            RustStruct::Tuple(lab, args) => Fragment::String(lab.clone()).cat(RustType::paren_list(args.iter())),
            RustStruct::Struct(lab, flds) => Fragment::String(lab.clone()).cat(Fragment::Char(' ')).cat(<(Label, RustType)>::block(flds.iter())),
        }
    }
}
...
enum AtomType<T = Box<RustType>, U = T> where T: Sized, U: Sized {
    Named(Label),
    Prim(PrimType),
    Comp(CompType<T, U>),
}
    ...
    fn value_type_to_fragment_with(vt: &ValueType, tycom: &mut TypeCompiler) -> Fragment {
        match vt {
            ...
            ValueType::Tuple(ts) => Fragment::seq(
                ts.iter()
                    .map(|vt| value_type_to_fragment_with(vt, tycom)),
    ...
    fn to_fragment(&self) -> Fragment {
        match self {
            ...
            RustExpr::FunctionCall(f, args) => {
                f.to_fragment().cat(ToFragment::paren_list(args))
            }
