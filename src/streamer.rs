...
pub enum Op {
    ...
    Branch(usize),
    Parallel(Vec<Block>),
    MatchTree(MatchTree, Vec<Target>),
    MatchExpr(Expr, Vec<(Pattern, Target)>),
    PushTuple(usize),
    TupleField,
    PushRecord(usize),
    RecordField(Label),
...
impl<'a> Compiler<'a> {
    ...
    pub fn compile_program(module: &FormatModule, format: &Format) -> Result<Program, String> {
        ...
        while let Some((f, next, n)) = compiler.compile_queue.pop() {
            let b = compiler.compile_format(f, next)?;
            compiler.program.blocks[n] = b;
        }
        Ok(compiler.program)
    }

    fn chain_block(&mut self, mut fst: Block, mut snd: Block) -> Block {
        fst.ops.append(&mut snd.ops);
        fst
    }

    fn queue_compile(&mut self, f: &'a Format, next: Rc<Next<'a>>) -> usize {
        let n = self.program.blocks.len();
        self.program.blocks.push(Block { ops: vec![] });
    ...
    pub fn compile_one(format: &Format) -> Result<Program, String> {
        ...
        Compiler::compile_program(&module, format)
    }

    fn concat_blocks(bs: Vec<Block>) -> (Vec<Op>, Vec<Target>) {
        let mut indices = Vec::with_capacity(bs.len());
        let mut offset = 0;
        for block in &bs {
            let size = block.ops.len() + 1;
            indices.push((offset, size));
            offset += size;
        }
        let mut mbs = Vec::with_capacity(bs.len());
        let mut ops = Vec::new();
        for (index, mut block) in bs.into_iter().enumerate() {
            let (offset, _) = indices[index];
            let mut remain = 1;
            for (_, size) in &indices[index + 1..] {
                remain += size;
            }
            ops.append(&mut block.ops);
            ops.push(Op::Jump(Target::Forward(remain)));
            mbs.push(Target::Forward(offset + 1));
        }
        (ops, mbs)
    }

    fn concat_blocks2(bs: Vec<(Pattern, Block)>) -> (Vec<Op>, Vec<(Pattern, Target)>) {
        let mut indices = Vec::with_capacity(bs.len());
        let mut offset = 0;
        for (_pattern, block) in &bs {
            let size = block.ops.len() + 1;
            indices.push((offset, size));
            offset += size;
        }
        let mut mbs = Vec::with_capacity(bs.len());
        let mut ops = Vec::new();
        for (index, (pattern, mut block)) in bs.into_iter().enumerate() {
            let (offset, _) = indices[index];
            let mut remain = 1;
            for (_, size) in &indices[index + 1..] {
                remain += size;
            ...
            ops.append(&mut block.ops);
            ops.push(Op::Jump(Target::Forward(remain)));
            mbs.push((pattern, Target::Forward(offset + 1)));
        }
        (ops, mbs)
    }

    fn compile_format(&mut self, format: &'a Format, next: Rc<Next<'a>>) -> Result<Block, String> {
        match format {
            Format::ItemVar(level, arg_exprs) => {
                ...
                let n = if let Some(n) = self.decoder_map.get(&(*level, next.clone())) {
                    *n
                    ...
                    let n = self.queue_compile(f, next.clone());
            ...
            Format::Variant(label, f) => {
                let s = self.compile_format(f, next.clone())?;
                Ok(self.chain_block(s, Block::op(Op::Variant(label.clone()))))
            }
            Format::Union(branches) => {
                ...
                for (index, f) in branches.iter().enumerate() {
                    let mut block = self.compile_format(f, next.clone())?;
                    block.ops.push(Op::Branch(index));
                    bs.push(block);
                    fs.push(f.clone());
                }
                if let Some(tree) = MatchTree::build(self.module, &fs, next.clone()) {
                    let (mut ops, mbs) = Self::concat_blocks(bs);
                    ops.insert(0, Op::MatchTree(tree, mbs));
                    Ok(Block::ops(ops))
                } else {
            ...
            Format::UnionNondet(branches) => {
                ...
                for (label, f) in branches {
                    let s = self.compile_format(f, next.clone())?;
                    ds.push(self.chain_block(s, Block::op(Op::Variant(label.clone()))));
                }
                Ok(Block::op(Op::Parallel(ds)))
            }
            Format::Tuple(fields) => {
                let mut chain = Block::op(Op::PushTuple(fields.len()));
                let mut fields = fields.iter();
                while let Some(f) = fields.next() {
                    let next = Rc::new(Next::Tuple(fields.as_slice(), next.clone()));
                    let s = self.compile_format(f, next)?;
                    chain = self.chain_block(chain, s);
                    chain = self.chain_block(chain, Block::op(Op::TupleField));
                }
                Ok(chain)
            }
            Format::Record(fields) => {
                let mut chain = Block::op(Op::PushRecord(fields.len()));
                let mut fields = fields.iter();
                while let Some((name, f)) = fields.next() {
                    let next = Rc::new(Next::Record(fields.as_slice(), next.clone()));
                    let s = self.compile_format(f, next)?;
                    chain = self.chain_block(chain, s);
                    chain = self.chain_block(chain, Block::op(Op::RecordField(name.clone())));
                }
                chain = self.chain_block(chain, Block::op(Op::PopRecord));
                Ok(chain)
            }
            Format::Repeat(a) => {
                ...
                let fb = Format::EMPTY;
                if let Some(tree) = MatchTree::build(self.module, &[fa, fb], next.clone()) {
                    let mut block = self.compile_format(a, Rc::new(Next::Repeat(a, next)))?;
                    let body_length = block.ops.len();
                    block.ops.insert(0, Op::PushSeq);
                    block.ops.insert(
                        1,
                        Op::MatchTree(
                            tree,
                            vec![Target::Forward(1), Target::Forward(body_length + 3)],
                        ),
                    );
                    block.ops.push(Op::SeqItem);
                    block.ops.push(Op::Jump(Target::Back(body_length + 2)));
                    let chain = Block::ops(block.ops);
                    Ok(chain)
                } else {
                    Err(format!("cannot build match tree for {:?}", format))
                }
            ...
            Format::Repeat1(a) => {
                ...
                if let Some(tree) = MatchTree::build(self.module, &[fa, fb], next.clone()) {
                    ...
                    block.ops.insert(0, Op::PushSeq);
                    block.ops.push(Op::SeqItem);
                    block.ops.push(Op::MatchTree(
                        tree,
                        vec![Target::Forward(1), Target::Back(body_length + 1)],
                    ));
                    Ok(Block::ops(block.ops))
                } else {
                    Err(format!("cannot build match tree for {:?}", format))
                }
            }
            Format::RepeatCount(expr, a) => {
                let mut block = self.compile_format(a, next)?;
                let body_length = block.ops.len();
                block.ops.insert(0, Op::PushSeq);
                block.ops.insert(1, Op::PushCounter(expr.clone()));
                block.ops.insert(
                    2,
                    Op::CounterZero(Target::Forward(body_length + 4), Target::Forward(1)),
                );
                block.ops.push(Op::SeqItem);
                block.ops.push(Op::DecCounter);
                block.ops.push(Op::Jump(Target::Back(body_length + 3)));
                Ok(Block::ops(block.ops))
            }
            Format::RepeatUntilLast(expr, a) => {
                ...
                block.ops.insert(0, Op::PushSeq);
                block.ops.push(Op::SeqItem);
                block.ops.push(Op::LambdaSeqLast(expr.clone()));
                block
                    .ops
                    .push(Op::If(Target::Forward(1), Target::Back(body_length + 2)));
                Ok(Block::ops(block.ops))
            }
            Format::RepeatUntilSeq(expr, a) => {
                ...
                block.ops.insert(0, Op::PushSeq);
                block.ops.push(Op::SeqItem);
                block.ops.push(Op::Lambda(expr.clone()));
                block
                    .ops
                    .push(Op::If(Target::Forward(1), Target::Back(body_length + 2)));
                Ok(Block::ops(block.ops))
            ...
            Format::Peek(a) => {
                let mut chain = Block::op(Op::PushInput);
                let s = self.compile_format(a, Rc::new(Next::Empty))?;
                chain = self.chain_block(chain, s);
                chain = self.chain_block(chain, Block::op(Op::PopInput));
                Ok(chain)
            }
            Format::PeekNot(a) => {
            ...
            Format::Slice(expr, a) => {
                let mut chain = Block::op(Op::PushInputSlice(expr.clone()));
                let s = self.compile_format(a, Rc::new(Next::Empty))?;
                chain = self.chain_block(chain, s);
                chain = self.chain_block(chain, Block::op(Op::PopInput));
                Ok(chain)
            ...
            Format::WithRelativeOffset(expr, a) => {
                let mut chain = Block::op(Op::PushInputOffset(expr.clone()));
                let s = self.compile_format(a, Rc::new(Next::Empty))?;
                chain = self.chain_block(chain, s);
                chain = self.chain_block(chain, Block::op(Op::PopInput));
                Ok(chain)
            ...
            Format::Map(a, expr) => {
                let s = self.compile_format(a, next.clone())?;
                Ok(self.chain_block(s, Block::op(Op::Map(expr.clone()))))
            }
            Format::Compute(expr) => Ok(Block::op(Op::Compute(expr.clone()))),
            Format::Let(name, expr, a) => {
                let mut chain = Block::op(Op::PushLet(name.clone(), expr.clone()));
                let s = self.compile_format(a, next.clone())?;
                chain = self.chain_block(chain, s);
                chain = self.chain_block(chain, Block::op(Op::PopLet));
                Ok(chain)
            }
            Format::Match(head, branches) => {
                let mut bs = Vec::with_capacity(branches.len());
                for (index, (pattern, f)) in branches.iter().enumerate() {
                    let mut block = self.compile_format(f, next.clone())?;
                    block.ops.push(Op::Branch(index));
                    block.ops.push(Op::PopPattern);
                    bs.push((pattern.clone(), block));
                }
                let (mut ops, mbs) = Self::concat_blocks2(bs);
                ops.insert(0, Op::MatchExpr(head.clone(), mbs));
                Ok(Block::ops(ops))
            }
            Format::Dynamic(name, dynformat, a) => {
                let mut chain = Block::op(Op::PushDynamic(name.clone(), dynformat.clone()));
                let s = self.compile_format(a, next.clone())?;
                chain = self.chain_block(chain, s);
                chain = self.chain_block(chain, Block::op(Op::PopDynamic));
                Ok(chain)
            }
        ...
        'blockloop: loop {
            match block {
                Block { ops } => {
                    let mut start = 0;
                    match 'oploop: loop {
                        for (ip, op) in ops[start..].iter().enumerate() {
                            match op {
                                ...
                                Op::MatchExpr(head, branches) => {
                                    let scope = &Scope::Other(ctxt);
                                    let head = head.eval(scope);
                                    for (pattern, target) in branches.iter() {
                                ...
                                Op::PushCounter(expr) => {
                                    let scope = &Scope::Other(ctxt);
                                    let n = expr.eval_value(scope).unwrap_usize();
                                    ctxt.counters.push(n);
                                }
                                Op::PopCounter => {
                                    ctxt.counters.pop().unwrap();
                                }
                                Op::DecCounter => {
                                    let mut n = ctxt.counters.pop().unwrap();
                                    n = n.checked_sub(1).unwrap();
                                    ctxt.counters.push(n);
                                }
                                Op::CounterZero(bzero, bnonzero) => {
                                    let target = if *ctxt.counters.last().unwrap() == 0 {
                                        bzero
                                    } else {
                                        bnonzero
                                    };
                                ...
                                Op::Lambda(expr) => {
                                    let arg = ctxt.values.last().unwrap();
                                    let scope = &Scope::Other(ctxt);
                                    let v = expr.eval_lambda(scope, arg);
                                    ctxt.values.push(v);
                                }
                                Op::LambdaSeqLast(expr) => {
                                    if let Value::Seq(vs) = ctxt.values.last().unwrap() {
                                        let arg = vs.last().unwrap();
                                        let scope = &Scope::Other(ctxt);
                                        let v = expr.eval_lambda(scope, arg);
                                        ctxt.values.push(v);
                                    } else {
                                ...
                                Op::If(btrue, bfalse) => {
                                    let target = if ctxt.values.pop().unwrap().unwrap_bool() {
                                        btrue
                                    } else {
                                        bfalse
                                    };
                                ...
                                Op::PushDynamic(
                                    ...
                                    let dyn_d = decoder::Compiler::compile_one(&f).unwrap();
