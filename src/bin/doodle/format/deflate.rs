    ...
    record([
        ...
        (
            ...
            Format::Compute(add(
                ...
                Expr::AsU16(Box::new(Expr::RecordProj(
                    Box::new(Expr::Var(0)),
                    "@value".to_string(),
                ))),
            )),
...
fn length_record(start: usize, base: &BaseModule, extra_bits: usize) -> Format {
    record([
        ...
        (
            "distance-code-value",
            Format::Compute(Expr::RecordProj(
                Box::new(Expr::UnwrapVariant(Box::new(Expr::Var(0)))),
                "@value".to_string(),
            )),
        ),
...
pub fn main(module: &mut FormatModule, base: &BaseModule) -> Format {
    ...
    let dynamic_huffman = module.define_format(
        ...
        record([
            ...
            (
                "code-length-alphabet-code-lengths-value",
                Format::Compute(Expr::FlatMap(
                    Box::new(Expr::Seq(vec![Expr::RecordProj(
                        Box::new(Expr::Var(0)),
                        "@value".to_string(),
                    )])),
                    Box::new(Expr::Var(0)),
                    ...
                    Expr::Gte(
                        ...
                        Box::new(add(
                            Expr::AsU16(Box::new(add(
                                Expr::RecordProj(Box::new(Expr::Var(5)), "@value".to_string()),
                                Expr::RecordProj(Box::new(Expr::Var(4)), "@value".to_string()),
                            ))),
                            Expr::U16(258),
                        )),
                    ),
